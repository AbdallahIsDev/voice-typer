...
The Tauri sidecar WebSocket IPC requires a per-session authentication token.
The token is generated by the Tauri Rust host and passed to the Python
backend via the `VOICE_TYPER_IPC_TOKEN` environment variable. No other local
process can connect without this token.
