...
cannot invoke arbitrary commands — only the **66** commands listed in
...
> `voice_typer/server/ipc_server.py` registers **68** handlers. Two of
...
> directly, NOT via the generic dispatch path). The remaining **66**
...
> they no longer exist in any layer. The current counts are 68 Python
> ↔ 66 TS ↔ 64 Rust, with the +2 host-only delta as the only
