...
cannot invoke arbitrary commands — only the **67** commands listed in
...
> `voice_typer/server/ipc_server.py` registers **69** handlers. Two of
...
> directly, NOT via the generic dispatch path). The remaining **67**
...
> they no longer exist in any layer. The current counts are 69 Python
> ↔ 67 TS ↔ 65 Rust, with the +2 host-only delta as the only
...
> the 17 removed in the cleanup pass.)
