...
pub(crate) fn require_main_window(window: &tauri::Window) -> Result<(), LausuError> {
    if !main_window_label_check(window.label()) {
        log::warn!(
            "[window-guard] command rejected from non-main window: {}",
            window.label()
        );
        return Err(LausuError::disallowed_main_window());
    }
    Ok(())
}
...
pub(crate) fn require_bubble_window(window: &tauri::Window) -> Result<(), LausuError> {
    if window.label() != "bubble" {
        log::warn!(
            "[window-guard] command rejected from non-bubble window: {}",
            window.label()
        );
        return Err(LausuError::disallowed_bubble_window());
    }
    Ok(())
}
