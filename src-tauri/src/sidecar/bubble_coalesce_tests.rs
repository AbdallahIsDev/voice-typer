//! Unit tests for `sidecar::bubble_coalesce`.
//!
//! Extracted from the inline `#[cfg(test)] mod tests { ... }` block that
//! previously lived at the bottom of `bubble_coalesce.rs`. The split
//! satisfies C-TEST-5 (no inline test code in production source files).
//!
//! Tests are wired via `#[cfg(test)] #[path = "bubble_coalesce_tests.rs"]
//! mod bubble_coalesce_tests;` declared in `bubble_coalesce.rs` (same
//! convention as `sidecar/spawn.rs` → `spawn_tests.rs`), so
//! `use super::*` below resolves to the `bubble_coalesce` module.
