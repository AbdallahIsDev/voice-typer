...
/**
 * XE-20-5 + XE-20-6 regression coverage for `rotateIfNeeded` +
 * `appendLogLine` permission handling.
 *
 * XE-20-5: `appendLogLine` previously called `fs.chmodSync(filePath,
 ...
 *
 * XE-20-6: `rotateIfNeeded` previously did NOT chmod the `.1` backup
...
describe("XE-20-5: appendLogLine chmod once per session (not per-write)", () => {
	...
	it("does NOT call chmodSync on the second append (Set gate)", async () => {
		...
		// XE-20-5: only ONE chmod call across two appends.
		expect(chmodCalls.length).toBe(1);
	});
