 ...
 *
 * DJ-50 regression coverage: `appendLifecycleLine` (the opt-in INFO
 ...
 * "avoid the XV-154 file-size cache — the INFO stream is lower priority
 ...
 *
 * The DJ-50 fix routes through `appendLogLine` which already uses the
 * XV-154 file-size cache (stat on cache miss only, bump after each
 * successful append).
 *
 * These tests verify:
...
// `electron-main.log` write fires (irrelevant to this test, but
// required for the structuredLogger module to load without error).
vi.mock("electron", () => ({
	app: {
		getPath: () => "/tmp/vt-dj-50-test-userdata",
		isPackaged: false,
	},
}));
