 ...
 *
 * XZ-LOG-03 regression coverage: the structured logger's `formatLine`
 ...
 *       `PERSIST_INFO=1`) ALSO redacts (XZ-LOG-03 lifecycle path).
...
describe("XZ-LOG-03: structuredLogger formatLine redacts PII / API keys / URL credentials", () => {
	...
	it("appendLifecycleLine (PERSIST_INFO=1) also redacts PII in the INFO stream", async () => {
		...
		// redacted (the XZ-LOG-03 fix ports redactPii into
