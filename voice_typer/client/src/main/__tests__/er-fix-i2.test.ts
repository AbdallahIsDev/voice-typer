 ...
 *
 * ER-FIX-I2 regression coverage for three findings:
 *
 *   (a) ER-63 — `getRuntimeLogPath()` memoization. Previously called
 ...
 *
 *   (b) ER-86 — `setupErrorHandlers` idempotency. Previously discarded
 ...
 *
 *   (c) ER-22 — `BUBBLE_ONLY_TYPES` shared constant. The five
...
const electronMocks = vi.hoisted(() => ({
	electronGetPathSpy: vi.fn(() => "/tmp/vt-er-fix-i2-userdata"),
}));
...
vi.mock("../single_instance", () => ({
	computeConfigDir: () => "/tmp/vt-er-fix-i2-userdata",
...
// ────────────────────────────────────────────────────────────────────
// (a) ER-63 — getRuntimeLogPath memoization
// ────────────────────────────────────────────────────────────────────
...
describe("ER-63: getRuntimeLogPath is memoized", () => {
	...
	it("returns the same string on every call", () => {
		...
		expect(a).toBe("/tmp/vt-er-fix-i2-userdata/electron-runtime.log");
	});
...
// ────────────────────────────────────────────────────────────────────
// (b) ER-86 — setupErrorHandlers disposes old handlers before installing new
// ────────────────────────────────────────────────────────────────────
...
describe("ER-86: setupErrorHandlers disposes old handlers before installing new", () => {
	...
	it("second call disposes the prior pair before installing the new pair (idempotent)", () => {
		...
		setupErrorHandlers();
		// ER-86: the second call must have issued `process.off`
	...
	it("does not accumulate listeners across repeated calls (no leak)", () => {
		...
		// ER-86: afterSecond == afterFirst (one new install, one
...
// ────────────────────────────────────────────────────────────────────
// (c) ER-22 — BUBBLE_ONLY_TYPES contains the 5 expected types
// ────────────────────────────────────────────────────────────────────
