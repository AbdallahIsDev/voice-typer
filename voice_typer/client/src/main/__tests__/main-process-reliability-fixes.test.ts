...
/**
 *  unit tests for GROUP 5 reliability fixes in the Electron
 * main-process lifecycle.
 *
 * Coverage:
 *   - :  SIGTERM/SIGINT handlers + 3s backstop (index.ts source-text)
 *   - :  synchronous SIGKILL in `_productionExit` (bootstrap source-text)
 *   - :  `.once('exit')` nulls `state.pythonProcess` (stop-python runtime)
 *   - :  killTimer is NOT `.unref()`'d + forceExitTimer removed (source-text)
 *   - : `crashReporter.start` + `child-process-gone` handler (runtime)
 *   - : `_resetStopPythonFlags()` resets the idempotency guard (runtime)
 *   - : `logEvent` logs to `console.error` on fs failure (runtime)
 *
...
// ────────────────────────────────────────────────────────────────────
//SIGTERM/SIGINT handlers in index.ts (source-text)
// ────────────────────────────────────────────────────────────────────
...
describe("GT-60: stop-python.ts .once('exit') nulls state.pythonProcess", () => {
	...
	it("nulls state.pythonProcess when Python exits gracefully (before killTimer fires)", () => {
		...
		//state.pythonProcess must be nulled so will-quit's
...
describe("GT-B3-8: bootstrap logEvent logs to console.error on fs failure", () => {
	...
	it.skip("trips the breaker with a log path that cannot be written (appendFileSync throws)", async () => {
		...
		//error. The logEvent catch () must call console.error.
		...
		//console.error must have been called with the logEvent
