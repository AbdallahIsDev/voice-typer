 ...
 *
 * Public API:
 *   - `recordRenderCrash(timestamps, label)` — internal helper; returns
 *     true when the sliding window is over the threshold.
...
export function recordMainWindowRenderCrash(): boolean {
	return recordRenderCrash(_mainWindowCrashTimestamps, "Main");
}
