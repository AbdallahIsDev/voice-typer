...
import { THEMES, applyThemeVars, deriveCustomVars, CUSTOM_COLOR_KEYS, DEFAULT_CUSTOM_LIGHT, DEFAULT_CUSTOM_DARK, type CustomThemeData } from '@/themes'
 ...
 */
function getCurrentThemeColors(currentPresetId: string): { light: Record<string, string>; dark: Record<string, string> } {
  const keys = CUSTOM_COLOR_KEYS.map(k => k.var)

  // Built-in preset with defined vars — read from THEMES array directly
  if (currentPresetId && currentPresetId !== 'default' && currentPresetId !== 'custom') {
...
export default function SettingsPage({ onThemeChange, onNavigate }: SettingsPageProps) {
  ...
  const lastNonCustomRef = useRef(
    config?.theme_preset && config.theme_preset !== 'custom' ? config.theme_preset : 'default',
  )
  ...
  const [customEditorMode, setCustomEditorMode] = useState<'light' | 'dark'>('light')
  ...
  useEffect(() => {
    ...
    if (draft) {
      ...
      setCustomDraft({ light: { ...DEFAULT_CUSTOM_LIGHT }, dark: { ...DEFAULT_CUSTOM_DARK } })
    }
  }, [config?.custom_theme === undefined]) // only on first load when config becomes available
    ...
    <div className="min-h-full">
      <div className="mx-auto max-w-2xl space-y-8 px-6 pt-28 pb-6">
        ...
        {_section_has_visible_items([
          { label: 'Color Scheme', info: 'Switch between light, dark, or follow your system setting.' },
          { label: 'Theme Preset', info: 'A built-in color scheme applied on top of your chosen mode.' },
          { label: 'Custom Theme', info: 'Create your own color scheme with the color picker.' },
          { label: 'Text Size', info: 'Adjust the UI text size for better readability.' },
        ]) && (
        <SettingsSection
          title="Appearance"
        ...
        >
          ...
          {_filter_settings('Color Scheme', 'Switch between light, dark, or follow your system setting.') && (
          <SettingRow label="Color Scheme" info="Switch between light, dark, or follow your system setting.">
            <Select
          ...
          {_filter_settings('Theme Preset', 'A built-in color scheme applied on top of your chosen mode.') && (
          <SettingRow label="Theme Preset" info="Choose a built-in color scheme. Hover to preview, click to apply permanently.">
            <Select
            ...
            >
              <SelectTrigger className="w-44" aria-label="Theme Preset">
                {(() => {
                  const current = THEMES.find((t) => t.id === (config.theme_preset ?? 'default')) ?? THEMES[0]
                  return (
          ...
          {_filter_settings('Custom Theme', 'Create your own color scheme with the color picker.') && (
          <SettingRow label="Custom Theme" info="Create your own color scheme — choose each color manually.">
            <Switch
              ...
              onCheckedChange={(enabled) => {
                if (enabled) {
                  ...
                  const currentColors = getCurrentThemeColors(config.theme_preset ?? 'default')
            ...
            <div className="animate-fade-in px-3.5 pb-4">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {CUSTOM_COLOR_KEYS.map(({ var: varName, label, description }) => {
                  const currentHex = customDraft[customEditorMode]?.[varName] ?? (
                    customEditorMode === 'light' ? DEFAULT_CUSTOM_LIGHT[varName] : DEFAULT_CUSTOM_DARK[varName]
                  )
                  return (
                    <div key={varName} className="flex items-center gap-2.5 rounded-lg border border-border bg-(--bg) p-2">
                      <div className="relative shrink-0">
                        <input
                          type="color"
                          value={currentHex}
                          onChange={(e) => handleCustomColorChange(customEditorMode, varName, e.target.value)}
              ...
              <button
                onClick={() => {
                  const defaults = { light: { ...DEFAULT_CUSTOM_LIGHT }, dark: { ...DEFAULT_CUSTOM_DARK } }
