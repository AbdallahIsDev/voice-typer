/**
 * SA-8 (client_pages) — regression tests for the fixes applied in this
 * sub-agent session. Each describe block pins one finding so a future
 * regression points at the exact contract that broke.
 *
 *   - S2-CR-39  Onboarding mic auto-select prefers `default: true`
 ...
 *               step.
 *   - S5-CR-104 History "Clear All" button has a permanent destructive
 *               visual cue (text-destructive + border-destructive at
 *               rest, not just on hover).
 *   - EC-12     Home.tsx extraction: the page imports the extracted
...
// ── S2-CR-39: mic auto-select prefers `default: true` ─────────────────
...
describe("S2-CR-39: Onboarding mic auto-select prefers default-flagged device", () => {
        ...
        it("auto-selects the default-flagged device when multiple mics are present", async () => {
                ...
                mockCall.mockImplementation((type: string, payload?: Record<string, unknown>) => {
                        switch (type) {
                                ...
                                case "onboarding_get_microphones":
                                        return Promise.resolve({
                                                microphones: [
                                                        ...
                                                        { id: "mic-3", name: "Bluetooth", default: false, is_bluetooth: true },
                                                ],
                                        });
        ...
        it("MicrophoneStep renders Default badge on the default-flagged device + BT badge on bluetooth device", async () => {
                                ...
                                case "onboarding_get_microphones":
                                        return Promise.resolve({
                                                microphones: [
                                                        { id: "mic-1", name: "USB Mic", default: false },
                                                        { id: "mic-2", name: "Built-in", default: true },
                                                        { id: "mic-3", name: "BT Headset", default: false, is_bluetooth: true },
                                                ],
                                        });
                                case "onboarding_get_hotkey_presets":
                ...
                expect(document.querySelector('[data-testid="mic-default-badge-mic-2"]')).not.toBeNull();
                expect(document.querySelector('[data-testid="mic-default-badge-mic-1"]')).toBeNull();
                // The BT badge exists for mic-3 only.
                expect(document.querySelector('[data-testid="mic-bluetooth-badge-mic-3"]')).not.toBeNull();
                expect(document.querySelector('[data-testid="mic-bluetooth-badge-mic-1"]')).toBeNull();
        });

        it("Onboarding.tsx source disables Continue when no mics detected on Microphone step", async () => {
                const fs = await import("node:fs");
                const src = fs.readFileSync("src/renderer/src/pages/Onboarding.tsx", "utf8");
                // S2-CR-39 (d): the Continue button's disabled prop must include
...
// ── EC-12: Home.tsx extraction preserves the R7-F13 contract ──────────
