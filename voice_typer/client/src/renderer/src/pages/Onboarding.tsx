...
export default function OnboardingPage({ onComplete }: OnboardingPageProps) {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const mics = await call<{ microphones: { id: string; name: string }[] }>('onboarding_get_microphones')
        setMicrophones(mics.microphones || [])
        if (mics.microphones?.length > 0) setSelectedMic(mics.microphones[0].id)

        const presets = await call<{ presets: string[] }>('onboarding_get_hotkey_presets')
        setHotkeyPresets(presets.presets || [])

        const models = await call<{ models: ModelOption[] }>('onboarding_get_model_options')
  ...
  const handleNext = useCallback(async () => {
    try {
      ...
      if (step?.step === 1) {
        await call('onboarding_set_microphone', { mic_id: selectedMic || null })
      } else if (step?.step === 2) {
    ...
    <div className="mx-auto flex min-h-full w-full max-w-lg flex-col items-center px-6 pt-28 pb-6">
      ...
      <div className="mb-8 w-full">
        <div className="mb-2 flex items-center justify-between text-xs text-(--text-muted)">
          <span>Step {step.step + 1} of {step.total_steps}</span>
      ...
      <div className="w-full rounded-xl border border-border bg-(--bg) p-8">
        {step.step === 0 && (
          <>
            <h1 className="mb-3 text-2xl font-bold text-(--text-primary)">Welcome to Voice Typer</h1>
            <p className="mb-6 text-sm text-(--text-muted)">
              Voice Typer is a premium offline voice-to-text utility that runs in your system tray.
            ...
            <ul className="mb-6 space-y-2 text-sm text-(--text-secondary)">
              <li className="flex items-center gap-2"><span className="text-accent">1.</span> Choose your microphone</li>
              <li className="flex items-center gap-2"><span className="text-accent">2.</span> Select a hotkey</li>
              <li className="flex items-center gap-2"><span className="text-accent">3.</span> Pick a transcription model</li>
            </ul>
          </>
        )}
        ...
        {step.step === 1 && (
          <>
            <h2 className="mb-3 text-lg font-semibold text-(--text-primary)">Choose Your Microphone</h2>
            <p className="mb-4 text-sm text-(--text-muted)">
              Select the microphone you want to use for dictation. You can change this later in Settings.
            </p>
            {microphones.length > 0 ? (
              <Select value={selectedMic} onValueChange={setSelectedMic}>
                <SelectTrigger className="w-full" aria-label="Select microphone">
                  <SelectValue placeholder="Select microphone" />
                </SelectTrigger>
                <SelectContent>
                  {microphones.map((mic) => (
                    <SelectItem key={mic.id} value={mic.id}>{mic.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ) : (
              <p className="text-sm text-(--text-muted)">No microphones detected. You can set one later in Settings.</p>
            )}
          </>
        )}
        ...
        {step.step === 2 && (
          <>
            <h2 className="mb-3 text-lg font-semibold text-(--text-primary)">Choose Your Hotkey</h2>
            <p className="mb-4 text-sm text-(--text-muted)">
              Select the keyboard shortcut you&apos;ll use to start and stop dictation. F2 is the default.
            </p>
            <Select value={selectedHotkey} onValueChange={setSelectedHotkey}>
              ...
              <SelectContent>
                {hotkeyPresets.map((hk) => (
                  <SelectItem key={hk} value={hk}>{hk.replace(/[<>]/g, '').toUpperCase()}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </>
        ...
        {step.step === 3 && (
          <>
            <h2 className="mb-3 text-lg font-semibold text-(--text-primary)">Choose Your Model</h2>
            <p className="mb-4 text-sm text-(--text-muted)">
              Pick a transcription model. Smaller models are faster; larger models are more accurate.
              The model downloads in the background after setup.
            </p>
        ...
        {step.step === 4 && (
          <>
            <h2 className="mb-3 text-lg font-semibold text-(--text-primary)">You&apos;re All Set!</h2>
            <p className="mb-4 text-sm text-(--text-muted)">
              Your Voice Typer is configured and ready. Press your hotkey ({selectedHotkey.replace(/[<>]/g, '').toUpperCase()}) to start dictating.
              The model will download and load in the background — this may take a minute on first run.
            </p>
            <div className="rounded-lg bg-(--bg-subtle) p-4 text-xs text-(--text-muted)">
              <p><strong>Microphone:</strong> {selectedMic || 'Default'}</p>
              <p><strong>Hotkey:</strong> {selectedHotkey.replace(/[<>]/g, '').toUpperCase()}</p>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="default" onClick={handleNext} aria-label={step.step === 4 ? 'Get started' : 'Continue'}>
