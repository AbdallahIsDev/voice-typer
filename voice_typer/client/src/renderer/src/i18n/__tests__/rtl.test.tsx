...
describe("I18N-3: RTL document direction", () => {
	beforeEach(() => {
		...
		// Clear the dir attribute so each test starts clean.
	...
	it("sets dir=rtl on document.documentElement when locale is Arabic", () => {
		act(() => {
	...
	it("sets dir=ltr on document.documentElement for non-RTL locales", () => {
	...
	it("flips dir back to ltr when switching from Arabic to English", () => {
	...
	it("renders a mounted component with the correct dir after setLocale", () => {
		// Mount while in Arabic.
		act(() => {
			setLocale("ar" as Locale);
		});
		render(<DirectionDisplay />);
		// The assertion: document.documentElement.dir is "rtl" for Arabic.
		expect(document.documentElement.dir).toBe("rtl");
	});
});
