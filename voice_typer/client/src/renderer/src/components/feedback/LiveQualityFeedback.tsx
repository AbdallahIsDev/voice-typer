...
export function LiveQualityFeedback({
	...
	// (otherwise SR may only speak the diff). The timer is rendered OUTSIDE
	// the <output> live region (and NOT wrapped in role="timer" /
	// aria-live) because rapid per-second updates would spam the SR
	// broadcast channel — the polite status region below announces ONLY
	// the meaningful voice/quality state, never the ticking clock.
	return (
		<div className="mt-2 space-y-2">
			...
			<output className="block space-y-2" aria-live="polite" aria-atomic="true">
