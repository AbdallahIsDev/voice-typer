	...
	return (
		<DialogPortal>
			...
			<DialogPrimitive.Content
				...
				className={cn(
					"group/dialog-content fixed top-1/2 left-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-6 rounded-lg bg-surface p-6 text-foreground ring-1 ring-foreground/5 duration-100 outline-hidden data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-md data-[size=lg]:max-w-xs data-[size=lg]:sm:max-w-xl dark:ring-foreground/10 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
					className,
				)}
				onOpenAutoFocus={(e) => {
...
function DialogHeader({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn(
				"grid place-items-center gap-2 text-center sm:place-items-start sm:text-start",
				className,
			)}
			{...props}
		/>
