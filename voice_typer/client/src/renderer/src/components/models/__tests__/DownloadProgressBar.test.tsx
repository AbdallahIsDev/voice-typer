...
describe("DownloadProgressBar, layout stability (no per-tick shift)", () => {
	afterEach(() => {
		cleanup();
	});

	it("keeps speed + ETA slots mounted with a placeholder when data is absent", () => {
		// Pause clears speed/ETA; live pushes repopulate them. If the
		// spans unmounted on every clear, the row would shift on every
		// pause/resume transition AND on every optimistic-clear cycle.
		// Absent data renders "—" in the same fixed-width slot instead.
		render(
			<DownloadProgressBar {...baseProps} speedBps={null} etaSeconds={null} />,
		);
		const dashes = screen.getAllByText("—", { exact: false });
		expect(dashes.length).toBeGreaterThanOrEqual(2);
	});

	it("uses tabular-nums on the status line and stat spans", () => {
		// Proportional digits change width every tick ("112.6" vs
		// "152.6"); tabular figures keep every push the same width.
		const { container } = render(<DownloadProgressBar {...baseProps} />);
		const tabular = container.querySelectorAll(".tabular-nums");
		expect(tabular.length).toBeGreaterThanOrEqual(4);
	});

	it("keeps the Pause/Resume toggle at a fixed width in both states", () => {
		// "Pause" vs "Resume" (up to 16 chars across locales) must not
		// move the Cancel button on every toggle.
		const { unmount } = render(
			<DownloadProgressBar {...baseProps} isPaused={false} />,
		);
		const pauseBtn = screen.getByRole("button", { name: "Pause download" });
		const pauseClass = pauseBtn.getAttribute("class") ?? "";
		unmount();
		render(<DownloadProgressBar {...baseProps} isPaused={true} />);
		const resumeBtn = screen.getByRole("button", { name: "Resume download" });
		const resumeClass = resumeBtn.getAttribute("class") ?? "";
		for (const cls of [pauseClass, resumeClass]) {
			expect(cls).toContain("min-w-28");
