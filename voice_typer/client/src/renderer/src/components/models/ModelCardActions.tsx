 ...
 *
 * PVT-003 fix #1: extracted from `pages/Models.tsx`'s 60-line nested
 ...
 *   4. (PVT-003 fix #7) Deps-installable + not depsOk → "Download Deps"
 *      button using existing `models.download.deps*` i18n keys.
 *
 * PVT-003 fix #11: the Select button now sets `aria-busy` while a
 ...
 *
 * PVT-003 fix #12: disabled Download buttons get a `title` attribute
...
/**
 * PVT-003 fix #12 helper: returns the "one download at a time" tooltip
...
/**
 * PVT-003 fix #11 helper: aria-label for the Select button. While a
...
export function ModelCardActions({
	...
	// ── Branch 4 (PVT-003 fix #7): deps-installable + not depsOk ───
	//
		...
		return (
			<div className="flex items-center gap-2 shrink-0">
				<Button
					...
					aria-label={t("models.download.depsAria", { name: model.name })}
					title={anyDownloading ? oneAtATimeTitle() : undefined}
				>
					<HugeiconsIcon
						icon={Download01Icon}
						strokeWidth={2}
		...
		return (
			<div className="flex items-center gap-2 shrink-0">
				<Button
					...
					onClick={onDownload}
					disabled={anyDownloading}
					aria-label={t("models.card.downloadAria", { name: model.name })}
					// PVT-003 fix #12: explain why the button is disabled
	...
	return (
		<div className="flex items-center gap-2 shrink-0">
			<Button
				...
				// PVT-003 fix #11: announce the in-flight selection state
