 ...
 *
 * Extracted from ``HotkeyPicker.tsx`` so the component is reduced to its
 * presentational shell (Button + DropdownMenu + output hint + error
 * ``<p>`` + Clear button) while this hook owns the keyboard-capture
 * lifecycle: refs that track held keys, the 30s auto-exit countdown,
 * the always-attached DOM listeners, the backend cancel subscription,
 * and the commit / cancel / error logic.
 *
 * The hook is intentionally a 1:1 lift of the original capture code —
 * every ref, state, effect and callback moves here verbatim so the
 * observable behaviour is byte-identical. See ``HotkeyPicker.tsx`` for
 * the historical comments on each invariant (ESC-FIX-*, HOTKEY-DEFER-*,
 * HOTKEY-MULTIKEY-*, HOTKEY-FULLMSG-*, etc.); the comments below are
 * the same ones relocated with the code they describe.
 */
...
export interface UseHotkeyCaptureResult {
	 ...
	 * through the same error state the capture session uses. The
	 * dropdown has no capture session to reset (it's a one-shot
	 * selection), so it just calls ``setError`` and bails on
...
export function useHotkeyCapture({
	...
	const commitModifierOnlyCombo = useCallback(() => {
		...
		if (mode === "single" && mods.length > 1) {
			...
			setError(t("hotkeyValidation.dictationKeyMustBeSingle", { label }));
		...
		const conflict = checkHotkeyConflict(newHotkey, value, occupiedHotkeys, t);
		if (conflict) {
			setError(conflict);
			resetCaptureSession();
			return;
