/**
 * ER-FIX-J2 regression tests for Group 2 (Performance & Resources) fixes.
 *
 * Covers:
 *  (a) ER-20 — i18n ``t()`` caches the per-key interpolation RegExp.
 *  (b) ER-23 — ``formatBytes`` caches ``Intl.NumberFormat`` instances.
 *  (c) ER-28 — ``closeAudioContext`` nulls the shared AudioContext.
 *  (d) ER-61 — ``useConnection`` only probes ``get_status`` after a
 *      5-minute gap with no backend push events.
 *
 * These tests are deliberately narrow: they verify the caching / gating
 * behaviour added by the ER-FIX-J2 wave, not the broader functional
...
// ───────────────────────────────────────────────────────────────────────
// (a) ER-20 — t() caches the per-key interpolation RegExp
// ───────────────────────────────────────────────────────────────────────
...
// ───────────────────────────────────────────────────────────────────────
// (b) ER-23 — formatBytes caches Intl.NumberFormat
// ───────────────────────────────────────────────────────────────────────
...
// ───────────────────────────────────────────────────────────────────────
// (d) ER-61 — useConnection probes only after a 5-minute event gap
// ───────────────────────────────────────────────────────────────────────
...
describe("ER-61: useConnection probes only after 5-minute event gap", () => {
	...
	it("does NOT probe within 5 minutes of a backend push event", async () => {
		...
		// refreshes the ``lastEventTs`` tracked by ER-61.
		act(() => {
