/**
 * TY-3 regression tests: `useAudioLevels` rAF loop is gated on
 * `mode === "recording"`.
 *
...
describe("TY-3: useAudioLevels rAF loop is gated on mode === recording", () => {
	it("does NOT call getComputedStyle per-frame after entering idle mode", async () => {
		...
		// Tick several frames — under the pre-TY-3 implementation, each
		// of these would have called getComputedStyle once. Post-TY-3,
		// zero calls expected.
		await tickFrames(10);
