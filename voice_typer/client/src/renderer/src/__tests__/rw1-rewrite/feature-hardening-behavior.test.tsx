/**
 * RW-1 vitest rewrite — behavioral tests for feature-hardening invariants.
 *
...
describe("Settings — RW-1 rewrite of test_settings_uses_shared_hook", () => {
	...
	it("calls showSnack via the shared useSnackbar hook when 'Open Log Folder' succeeds (delegates to sonner.toast.success, not inline state)", async () => {
		...
		// If Settings had inline snackbar state (the pre-NEW-TS-004
...
describe("Microphone — RW-1 rewrite of test_microphone_uses_shared_snackbar_hook", () => {
	...
	it("calls showSnack via the shared useSnackbar hook when the mic test fails (delegates to sonner.toast.error, not inline JSX)", async () => {
		...
		// (the pre-NEW-TS-004 pattern), toast.error would never be called.
		await waitFor(() => {
...
describe("App — RW-1 rewrite of test_app_does_not_use_is_ready", () => {
	...
	it("renders a child page when usePython() returns { call } only (no isReady gate)", async () => {
		...
		// rendering on it (the pre-NEW-TS-015 pattern), the gate would
