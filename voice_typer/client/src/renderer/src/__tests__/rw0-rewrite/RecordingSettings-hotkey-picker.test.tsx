/**
 * RW-0 vitest rewrite — behavioral tests for `RecordingSettingsSection.tsx`
 * HotkeyPicker wiring.
 *
 ...
 *   2. Each rendered instance has mode="combo".
...
vi.mock("@hugeicons/core-free-icons", () => {
	...
	// new icon.  This is acceptable for tests that don't assert on
	// icon identity.
	return new Proxy(
		{
...
describe("RecordingSettings repaste HotkeyPicker — RW-0 rewrite of test_repaste_key_uses_hotkey_picker_combo_mode", () => {
	...
	it('renders the dictation-key HotkeyPicker with mode="combo" (matches repaste-key mode)', () => {
		// The dictation key is ALSO rendered via HotkeyPicker
		// in combo mode (a recent change from single → combo
		// so users can pick multi-key dictation triggers).
		render(
			<RecordingSettingsSection
