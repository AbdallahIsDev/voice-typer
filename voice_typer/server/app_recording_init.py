...
Owns the recording-subsystem startup slice of ``VoiceTyperApp``:
    ...
    - ``_init_recording``: declare the ``_recorder_backing`` /
      ...
      is in flight is never clobbered), and eagerly pre-import
      numpy/the recording package on the MAIN thread first (avoids the
      Python 3.13+ ``_ModuleLock`` import-lock deadlock).

    The eager Silero VAD preload that used to be spawned here is owned
    by ``StartupSequence`` phase 1
    (``startup_sequence/_phases_early.py``). ONE spawn site per boot,
    armed when ``app.start()`` runs the startup sequence (the only
    launcher in every production mode). The former construction-time
    ``vad-preload`` worker was a duplicate of the same best-effort
    preload (``vad.preload()`` is idempotent; the lazy-load fallback in
    ``compute_vad_prob`` is preserved either way).

Previously the builder lived on ``VoiceTyperApp`` in ``app.py``. The
