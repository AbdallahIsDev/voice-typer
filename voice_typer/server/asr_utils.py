...
# ─── Model-cache size cap (eviction) ─────────────────────────────────────

# Cap on the total on-disk size of the HuggingFace model cache under
# ``<config_dir>/huggingface/hub``. Enforced by :func:`prune_model_cache`
# (oldest-first eviction) so the cache cannot grow without bound —
# ``tiny.en`` + ``small.en`` + ``parakeet`` together already exceed
# 3 GB, and a user who cycles through every model would otherwise
# accumulate 10+ GB of weights. Pinned by
# ``tests/test_hf_cache_prune.py::test_default_max_bytes_is_8gb``.
_MAX_MODEL_CACHE_GB = 8
_MAX_MODEL_CACHE_BYTES = _MAX_MODEL_CACHE_GB * 1024**3


def _repo_size_bytes(repo_dir) -> int:
    """Total on-disk size (bytes) of a cached-repo directory, recursive.

    Best-effort: unreadable files are skipped (counted as 0 bytes) so a
    single permission-denied blob does not abort the eviction scan.
    """
    total = 0
...
def _prune_oldest_repos(repos, max_bytes: int, shutil) -> int:
    """Delete cached repos oldest-first until total size ≤ ``max_bytes``.

    ``repos`` is a list of ``(repo_path, size_bytes, sort_key)`` tuples
    where a LOWER ``sort_key`` means OLDER (and thus evicted first).
    Returns the number of repos actually deleted.  A single repo that is
    itself larger than the cap is still deleted (best-effort — the cap
    is a target, not a hard guarantee against one oversized model).
    """
    if not repos:
        return 0
    total = sum(size for _path, size, _key in repos)
    if total <= max_bytes:
        return 0
    pruned = 0
    for repo_path, size, _key in sorted(repos, key=lambda item: (item[2], str(item[0]))):
        if total <= max_bytes:
            break
        try:
            ...
            log.warning(
                "[MODEL-CACHE] failed to prune cached model %s: %s",
                repo_path,
                exc,
            )
    return pruned
...
def prune_model_cache(cache_dir, max_bytes: int = _MAX_MODEL_CACHE_BYTES) -> int:
    """Enforce a size cap on the HuggingFace model cache directory.

    Deletes the OLDEST cached repos (by ``last_modified`` / mtime) until
    the total on-disk size of ``cache_dir`` is under ``max_bytes``.
    Returns the number of repos deleted (0 when already under the cap,
    when the cache dir does not exist, or when it is empty).

    Uses ``huggingface_hub.scan_cache_dir`` when available (authoritative
    per-repo sizes + last-modified timestamps).  Falls back to a manual
    ``iterdir`` + ``stat().st_mtime`` scan when ``huggingface_hub`` is
    not importable or ``scan_cache_dir`` raises at call time (e.g. the
    cache layout is not a recognised HF layout) — the fallback must
    still evict oldest-first (pinned by
    ``tests/test_hf_cache_prune.py``).

    Best-effort: a single un-deletable repo (file lock, permission
    denied) is logged and skipped; the remaining repos are still
    evicted. Never raises.

    Parameters
    ----------
    cache_dir : str or Path
        The HuggingFace hub cache root (contains ``models--*`` dirs).
    max_bytes : int
        Size cap in bytes. Defaults to ``_MAX_MODEL_CACHE_BYTES``
        (8 GiB).

    Returns
    -------
    int
        Number of cached repos deleted.
    """
    import shutil
    ...
    # Preferred path: huggingface_hub.scan_cache_dir.
    repos = None
    try:
        from huggingface_hub import scan_cache_dir

        try:
            info = scan_cache_dir(str(cache_root))
            repos = []
            for r in info.repos:
                mtime = r.last_modified
                sort_key = mtime.timestamp() if mtime is not None else float("inf")
                repos.append((Path(r.repo_path), int(r.size_on_disk), sort_key))
        except ImportError:
            # ``scan_cache_dir`` itself is unavailable at runtime — fall
            # through to the manual scan below (contract pinned by
            # ``test_prune_fallback_when_scan_cache_dir_raises_importerror``).
            repos = None
        except Exception as exc:  # noqa: BLE001 — best-effort cache scan
            log.debug(
                "[MODEL-CACHE] scan_cache_dir failed (%s); using manual mtime scan",
                type(exc).__name__,
            )
            repos = None
    ...
    # Fallback: manual mtime-based scan (huggingface_hub not installed
    # or its scan_cache_dir failed at runtime).
    repos = []
    try:
        for entry in cache_root.iterdir():
            ...
            try:
                ...
                # Unknown age — treat as newest so we don't delete
                # un-datable data while known-old repos exist.
                mtime = float("inf")
            repos.append((entry, _repo_size_bytes(entry), mtime))
    except OSError:
        return 0
    return _prune_oldest_repos(repos, max_bytes, shutil)
