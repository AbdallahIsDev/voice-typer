...
def _drain_live_thread_registries() -> None:
    ...
    threads registered on ``app._thread_registry`` (e.g. the
    ``vad-preload-startup`` worker armed by ``StartupSequence`` phase 1,
    join_timeout=2.0, the ``vad-preload`` worker formerly armed by
    ``VoiceTyperApp.__init__``'s now-removed ``_preload_vad_model``
    helper is gone, leaving phase 1 the single spawn site). Under xdist a
    worker process lives for the WHOLE
