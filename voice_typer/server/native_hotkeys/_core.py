...
class SubprocessHotkeyBackend(_SpawnMixin, _ReaderMixin, _WatchdogMixin, _MatchingMixin, HotkeyBackend):
    ...
    def __init__(self, hotkey_str: str, binary_path: Path | None = None):
        ...
        # to the binary via ``--log-file <path>`` (appended to the spawn
        # command in ``_spawn_process``; the binary also accepts it as
        # positional argv[2]). The binary writes timestamped diagnostic
        # lines (init steps, permission checks, device opens, hook
        # installation, warnings) to this file so support bundles can
        # include a native-side diagnostic trace alongside the
        # Python-side log. None until ``_compute_native_log_path``
        # resolves it lazily on first spawn (so tests that construct
        # backends without spawning don't create log files); once
        # resolved it is memoised so watchdog respawns and restarts
        # append to the same file (``native-<backend>.log``, no PID).
