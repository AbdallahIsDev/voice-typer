...
class TrayController(Protocol):
    ...
    # UX-1 (FIX-10): ``undo_last`` IPC command was wired but unreachable
    # from any UI. The tray menu's new "Undo Last" item calls
    # ``controller.undo_last()`` (implemented on VoiceTyperApp at
    # app.py:650 — sends backspace keystrokes to remove the last
    # transcription). Declared here so pyrefly's structural Protocol
    # check accepts the new tray menu callback.
