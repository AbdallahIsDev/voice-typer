...
PVT-G5-094 (FA16, 2026-07-19): the ack envelope previously diverged
...
class RepasteHandlersMixin(HandlerBase):
    ...
    CR-20: this mixin's ``except Exception`` catch-all calls
    ...
    def _handle_repaste_last(self, data: dict | None, resp: dict) -> dict | None:
        ...
        PVT-G5-094: the ``"ok": True`` key was dropped — it was the
