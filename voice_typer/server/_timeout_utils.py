...
UE-21 / UE-11-F7: ``_run_with_timeout`` tracks leaked worker threads
...
UE-11-F8: ``_run_parallel_with_timeout`` raises ``ValueError`` if any
...
UE-11-F9: ``__all__`` exposes the canonical public names only.
...
# UE-11-F9: back-compat alias. Older tests (e.g.
...
_TIMEOUT = TIMEOUT
# UE-11-F9: back-compat alias. Same rationale as ``_TIMEOUT``.
...
# UE-21 / UE-11-F7: module-level registry of leaked worker threads.
#
...
def join_leaked_workers(timeout: float = 1.0) -> int:
    ...
    UE-21 / UE-11-F7: when ``_run_with_timeout`` returns ``TIMEOUT``,
...
def _run_with_timeout(description: str, func, timeout: float = 5.0):
    ...
    UE-21 / UE-11-F7: if the worker times out, it is added to the
    ...
    t.join(timeout=timeout)
    if t.is_alive():
        # UE-21 / UE-11-F7: track the leaked worker so the shutdown
...
def _run_parallel_with_timeout(
    ...
    UE-11-F8: *items* MUST have unique ``desc`` values. The result is
    ...
    if not items:
        return []
    # UE-11-F8: enforce uniqueness. The dict-based reorder below would
...
__all__ = [
    ...
    # (UE-11-F9) — new callers should use ``TIMEOUT`` /
    # ``SHUTDOWN_WATCHDOG_TIMEOUT_S``.
]
