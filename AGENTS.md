...
Rationale: A prior "make it prettier" edit changed the ring to `ring-ring/30`; a programmatic WCAG audit caught the 1.15:1–2.45:1 composite contrast (invisible in every theme). The fix was to drop the `/30` alpha, and `focus-ring-contrast.test.tsx` now pins the full-opacity contract + the `ring-1` thickness + the `focus-visible:` qualifier. Thickness is pinned at `ring-1` per user decision (2026-09-15); do not retune without one. NEVER touch the alpha. Established 2026-08-28.
Applies to: All agents, all modes, all sub-agents.
```

```
C-FOCUS-5
Rule: Do NOT remove or hide the focus ring, and do NOT use anything thinner than `ring-1` (1px). `ring-1` is the app's standard thickness (pinned by `focus-ring-contrast.test.tsx`) per explicit user decision (2026-09-15): the indicator meets WCAG 2.4.7 "Focus Visible" (Level A — a visible indicator is always present at full opacity) but is knowingly below WCAG 2.4.13 "Focus Appearance" (Level AAA — 2px minimum area). Aesthetics over AAA compliance is the owner's call, recorded here so no future session "fixes" it back. Never remove the ring, never a sub-1px hairline, never a `border` that visually collapses. Thickness is a SEPARATE axis from color opacity (C-FOCUS-2): full `ring-ring` opacity is non-negotiable. Do not retune thickness without a user decision.
Rationale: WCAG 2.4.13 requires the focus indicator's contrasting area to be at least as large as a 2px-thick perimeter; a 1px ring reads faintly for low-vision users, which is why the standard used to be thicker. The programmatic audit that fixed C-FOCUS-2 set full opacity; the user set `ring-1` on 2026-09-15 for minimum visual weight, accepting the AAA gap (AA 2.4.7 remains met). `focus-ring-contrast.test.tsx` pins `ring-1` + full opacity + `focus-visible:` so drift in either direction fails loudly. Established 2026-08-28; thickness amended 2026-09-15 by user decision.
Applies to: All agents, all modes, all sub-agents.
```
...
```
C-TDEV-2
Rule: Do NOT let the Tauri host call `app.restart()` in response to the sidecar's `relaunch_app` event when a dev-mode sidecar is running (`dev_mode::is_dev_mode()` in `src-tauri/src/sidecar/lifecycle.rs::on_relaunch_app`). In dev the host is the `tauri dev` CLI's child: `app.restart()` ends the CLI session and the relaunched exe is reaped by the CLI's Windows job object. The whole app dies (observed 2026-08-30: tray → Restart killed the entire dev session). Dev contract: the host stays up and the supervisor's WS-close respawn path restarts the exiting sidecar. Production (release, frozen externalBin) keeps `app.restart()`.
Rationale: Tray → Restart is a first-class user action; in dev it must restart the BACKEND, not murder the dev session. Symmetric guard tests live with the lifecycle code; the Python side handles its own exit (`restart_app` → cleanup → `sys.exit(0)`).
Applies to: All agents, all modes, all sub-agents.
```
...
```
C-MODELS-2
Rule: Do NOT let model-size download buttons resize to their content, center their content, or use an oversized icon. Every model-size Download button MUST use the shared tokens exported from `voice_typer/client/src/renderer/src/components/models/ModelCardActions.tsx`: fixed width `DOWNLOAD_SIZE_BUTTON_WIDTH` (`w-24`), left alignment `DOWNLOAD_CONTENT_ALIGNMENT` (`justify-start`, icon + size text share one start position across every row, overriding the Button base's `justify-center`), and the compact icon size `DOWNLOAD_ICON_CLASS` (`h-3 w-3`) so the download icon visually matches the 11px size text. Do NOT hardcode per-model widths, do NOT center the content, do NOT reintroduce the 16px `h-4` icon, and do NOT remove the `~`-stripping / number+unit-spacing normalization in `formatModelSize` (`voice_typer/client/src/renderer/src/lib/utils/models.ts`). Sizes must render as `75 MB` / `3 GB` / `809 MB`, no `~`, space always present. The "Download Deps" button (a label, not a size) is exempt from the fixed width but keeps the same left alignment.
