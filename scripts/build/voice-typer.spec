...
before running `cargo tauri build`. `nuitka_freeze.sh` does this for the
Nuitka path; for the PyInstaller fallback, copy manually:
...
`src-tauri/src/sidecar/spawn.rs::spawn_sidecar_release`).
"""
...
exe = EXE(
    ...
    a.datas,
    [],
    # Tauri sidecar: emit `python-sidecar-<triple>` (console on, so Rust
    # can read server_started JSON from stdout).
    name=_EXE_NAME,
    debug=False,
    ...
    # Tauri sidecar uses no icon (it's a background process; Tauri's host
    # exe carries the app icon).
    icon=None,
