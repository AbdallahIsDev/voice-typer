#!/bin/bash
#Lausu, macOS uninstall cleanup script ( + ).
#
...
#
#removes ~/Library/LaunchAgents/com.Lausu.plist (the
...
#
#removes ~/Library/Application Support/lausu (the per-user
# data dir: settings JSON, history DB, downloaded vocabularies, etc.).
# This is the macOS equivalent of ~/.local/share/lausu on Linux.
#
...
#       bash /Applications/lausu.app/Contents/Resources/scripts/macos/uninstall.sh
#
...
#         ls -l ~/Library/LaunchAgents/com.Lausu.plist
#         launchctl list | grep com.Lausu
#   5. Verify the data dir exists:
#         ls -la ~/Library/Application\ Support/lausu/
#   6. Drag lausu.app to Trash (or `rm -rf /Applications/lausu.app`).
#   7. Run this script:
#         bash scripts/macos/uninstall.sh
#   8. Verify the LaunchAgent + data dir are gone:
#         ls ~/Library/LaunchAgents/com.Lausu.plist   # No such file
#         ls ~/Library/Application\ Support/lausu/   # No such directory
#         launchctl list | grep com.Lausu             # no matches
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo ""
            echo "Removes per-user Lausu artifacts on macOS:"
            echo "  - ~/Library/LaunchAgents/com.Lausu.plist (autostart)"
            echo "  - ~/Library/Application Support/lausu (user data)"
            echo "  - ~/.cache/huggingface (only with --purge)"
            exit 0
            ;;
        *)
            echo "[lausu-uninstall] WARNING: unknown argument: $arg" >&2
            ;;
    esac
done
...
if [ -f "$PLIST_PATH" ]; then
    ...
    launchctl bootout "gui/${UID_NUM}/com.Lausu" 2>/dev/null || true
    launchctl remove "com.Lausu" 2>/dev/null || true
    rm -f "$PLIST_PATH"
    echo "[lausu-uninstall] Removed LaunchAgent: $PLIST_PATH"
else
    echo "[lausu-uninstall] No LaunchAgent to remove ($PLIST_PATH not present)"
fi

# Prewarm LaunchAgent cleanup.
# The prewarm scheduler (voice_typer/server/prewarm_scheduler_posix.py) registers
# a SEPARATE LaunchAgent at ~/Library/LaunchAgents/com.Lausu.prewarm.plist
# with label `com.Lausu.prewarm` (RunAtLoad=true). This is distinct from
...
# above.
PREWARM_PLIST="$HOME/Library/LaunchAgents/com.Lausu.prewarm.plist"
if [ -f "$PREWARM_PLIST" ]; then
    ...
    # login.
    launchctl bootout "gui/${UID_NUM}/com.Lausu.prewarm" 2>/dev/null || \
        launchctl unload "$PREWARM_PLIST" 2>/dev/null || true
    rm -f "$PREWARM_PLIST"
    echo "[lausu-uninstall] Removed prewarm LaunchAgent: $PREWARM_PLIST"
else
    echo "[lausu-uninstall] No prewarm LaunchAgent to remove ($PREWARM_PLIST not present)"
fi

#remove the per-user data directory.
# Quote the path to handle the space in "Application Support".
DATA_DIR="$HOME/Library/Application Support/lausu"
if [ -d "$DATA_DIR" ]; then
    rm -rf "$DATA_DIR"
    echo "[lausu-uninstall] Removed user data directory: $DATA_DIR"
else
    echo "[lausu-uninstall] No user data directory to remove ($DATA_DIR not present)"
fi
...
if [ "$PURGE" = "1" ]; then
    ...
    if [ -d "$HF_CACHE" ]; then
        rm -rf "$HF_CACHE"
        echo "[lausu-uninstall] Removed HuggingFace cache: $HF_CACHE"
    else
        echo "[lausu-uninstall] No HuggingFace cache to remove ($HF_CACHE not present)"
    fi
fi

echo "[lausu-uninstall] Done."
