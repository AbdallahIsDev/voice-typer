...
class _FakeApp:
    ...
    def __init__(self) -> None:
        ...
        # if the old (pre-DE-7) code path is still being used.
...
# ── DE-7: pop_streaming_session (atomic) ──────────────────────────────


class TestDE7PopStreamingSessionAtomic:
...
# ── DE-10: flush ordering ──────────────────────────────────────────────


class TestDE10FlushBeforeTeardown:
...
# ── DE-11: non-main-thread force-exit + tray.stop log escalation ──────


class TestDE11ForceExitOnNonMainThread:
...
# ── DE-53: _electron_pid lock ─────────────────────────────────────────


class TestDE53ElectronPidLock:
...
# ── DE-54: sd.stop() skipped on recorder.stop() timeout ───────────────


class TestDE54SkipSdStopOnRecorderTimeout:
