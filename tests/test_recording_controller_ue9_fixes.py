...
def _make_controller() -> RecordingController:
    ...
    ``test_recording_controller_de_fixes.py`` so the same code paths
    ...
    app._busy_event.set()  # not busy = set (inverted semantics, UE-9-F8)
    app._cycle_counter = 0
    app._cycle_id = "#0"
    app.recorder = MagicMock()
    app.recorder.recording = False
    app.recorder.last_rms = 0.0
    # UE-9-F6: ``_stop_impl`` reads ``_dropped_ring_chunks``; default to 0
...
# ─── UE-9-F1: _stop_impl pops + cancels the streaming session ────────────


class TestUE9F1StopImplPopsAndCancelsStreamingSession:
    """UE-9-F1: ``_stop_impl`` must use the atomic
    ``_cancel_streaming_session()`` helper (pop + public ``cancel()``)
    instead of get + private-attr poke."""

    def test_stop_pops_session_from_slot(self):
        """After ``stop()``, ``get_streaming_session()`` must return None
        (the session was popped, not just signalled)."""
        ...
        assert ctrl.get_streaming_session() is None, (
            ...
            "Pre-fix, the stale session reference persisted in "
            "self._streaming_session across the entire transcription window."
        )

    def test_stop_calls_public_cancel_not_private_attr_poke(self):
        """``stop()`` must call ``session.cancel()`` (public method), NOT
        poke ``session._cancel_event.set()`` (private attribute)."""
        ctrl, app = _make_controller()
        app.recorder.recording = True
        app.recorder.stop.return_value = b"\x00" * 16000

        fake_session = MagicMock()
        # Give the session a real _cancel_event so we can detect if the
        # OLD private-attr poke path is used (the event would be set).
        ...
        # The public cancel() must have been called.
        assert fake_session.cancel.called, "UE-9-F1: stop() must call session.cancel() (public method)."


# ─── UE-9-F3: _start_watchdog_thread holds _watchdog_lock ────────────────


class TestUE9F3StartWatchdogThreadHoldsLock:
...
# ─── UE-9-F6: _stop_impl logs WARNING for dropped ring chunks ────────────


class TestUE9F6DroppedRingChunksWarning:
...
# ─── UE-9-F15: _toggle_impl does not increment cycle counter for blocked toggles ──


class TestUE9F15CycleCounterNotIncrementedForBlockedToggles:
...
# ─── UE-9-F8: inverted _busy_event semantics documentation (deferred) ────


class TestUE9F8BusyEventSemanticsDocumented:
