...
class TestDepsProbesUseFindSpec:
    ...
    def test_parakeet_deps_uses_find_spec_source(self):
        """Source guard: ``_check_parakeet_deps`` must use
        ...
        assert "importlib.util.find_spec" in src, (
            "XV-1: _check_parakeet_deps must use importlib.util.find_spec "
            "(not import_module) to avoid executing torch's top-level code."
        )
        # Check for an actual CALL (with open paren) — the docstring may
        # legitimately mention ``importlib.import_module`` by name when
        ...
        assert "import_module(" not in src, (
            ...
            "which allocates GPU memory and inits CUDA just to probe presence."
        )

    def test_qwen_deps_uses_find_spec_source(self):
        """Source guard: ``_check_qwen_deps`` must use
        ``importlib.util.find_spec`` and must NOT CALL
        ``importlib.import_module(...)``."""
        from voice_typer.server.service import VoiceTyperService
    ...
    def test_parakeet_deps_returns_bool(self, tmp_config_dir):
        ...
        for ``deps_ok`` in the models status payload)."""
        from voice_typer.server.service import VoiceTyperService
    ...
    def test_qwen_deps_returns_bool(self, tmp_config_dir):
        """``_check_qwen_deps`` must return a ``bool``."""
        from voice_typer.server.service import VoiceTyperService
    ...
    def test_parakeet_deps_true_when_spec_present(self, tmp_config_dir, monkeypatch):
        ...
        ``_check_parakeet_deps`` must return True — WITHOUT calling
        ``importlib.import_module``."""
        ...
        # Fake find_spec — returns a truthy ModuleSpec stand-in.
        fake_spec = MagicMock(name="ModuleSpec")
        fake_find_spec = MagicMock(return_value=fake_spec)
        # Patch ``importlib.util.find_spec`` at the call site. The
        # function imports ``importlib.util`` locally, so we patch the
        # ``find_spec`` attribute on the real ``importlib.util`` module.
    ...
    def test_parakeet_deps_false_when_spec_missing(self, tmp_config_dir, monkeypatch):
        ...
        installed), ``_check_parakeet_deps`` must return False."""
        from voice_typer.server import service as svc_mod
    ...
    def test_qwen_deps_true_when_spec_present(self, tmp_config_dir, monkeypatch):
        ...
        monkeypatch.setattr(_ilu, "find_spec", lambda name: MagicMock() if name == "qwen_asr" else None)
        svc = svc_mod.VoiceTyperService(FakeApp())
