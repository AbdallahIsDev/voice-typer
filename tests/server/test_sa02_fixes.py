...
# ── XZ-IPC-001: main() sets _tcp_mode unconditionally ──────────────────


class TestXZIPC001StandaloneStdinSuppression:
    ...
    def test_start_spawns_stdin_thread_when_tcp_mode_false(self) -> None:
        """Counter-test: with ``_tcp_mode = False`` (the legacy stdin
        console path used by some tests), ``start()`` DOES spawn the
        stdin listener thread. This pins the branch logic so the
        ...
        try:
            ...
            assert srv2._stdin_thread is not None, (
                "Counter-test for XZ-IPC-001: with _tcp_mode=False, "
                "start() must spawn the stdin listener thread (legacy "
                "stdin/stdout console path)."
            )
        finally:
            srv2.stop()


# ── ZR-76: _send_stdin_error_envelope helper ───────────────────────────


class TestZR76SendStdinErrorEnvelope:
...
# ── XZ-IPC-009: stale line-number references removed ───────────────────


class TestXZIPC009StaleLineRefs:
