...
def test_gp91_event_bus_count_is_36_in_doc_and_code():
    ...
    doc = _read(ARCH_DOC)
    assert "36-event bus" in doc, (
        "ARCHITECTURE.md must describe the bus as '36-event bus' ."
    )
    assert "24-event bus" not in doc, (
...
def test_gp92_capabilities_row_lists_accurate_perms():
    ...
    doc = _read(ARCH_DOC)
    caps_row_match = re.search(
        r"\| Capabilities \|.*?\|\s*(?P<body>[^|]+)\s*\|", doc
    )
    ...
    assert "clipboard-manager" in body, (
        "capabilities row must mention clipboard-manager's removal "
        "(XE-4-4) so future readers know why it is absent."
    )
    ...
    for stale_tray_perm in [
        ...
        assert stale_tray_perm not in body, (
            f"granular tray perm '{stale_tray_perm}' was dropped — must "
            f"not be listed as an active grant."
        )
    ...
    for claim in required_claims:
        assert claim in body, (
            f"capabilities row must mention '{claim}'. Row body: {body!r}"
        )
    ...
    assert set(perms) == expected_perms, (
        f"main-runtime.json permissions drift. Expected {sorted(expected_perms)}, "
        f"got {sorted(perms)}."
    )
...
def test_gp93_cargo_manifest_row_does_not_mention_removed_deps():
    doc = _read(ARCH_DOC)
    cargo_row_match = re.search(
        r"\| Cargo manifest \|.*?\|\s*(?P<body>[^|]+)\s*\|", doc
    )
    ...
    assert "clipboard-manager" in body, (
        "Cargo manifest row must mention clipboard-manager's removal (XE-4-4)."
    )
    assert "enigo" in body, (
        "Cargo manifest row must mention enigo's removal (FZ-19)."
    )
    assert "were removed (XE-4-4 / FZ-19)" in body, (
        "row must say 'were removed (XE-4-4 / FZ-19)'."
    )
    ...
    dep_pattern = re.compile(
        r'^tauri-plugin-clipboard-manager\s*=', re.MULTILINE
    )
    ...
    assert not enigo_dep_pattern.search(cargo), (
        "enigo must NOT be declared as a dependency in Cargo.toml (FZ-19)."
    )
...
def test_gp94_tauri_command_count_in_doc_matches_code():
    doc = _read(ARCH_DOC)
    rust_row_match = re.search(
        r"\| Rust host \|.*?\|\s*(?P<body>[^|]+)\s*\|", doc
    )
    ...
    assert "18 Tauri commands" in body, (
        f"Rust host row must say '18 Tauri commands'. Got: {body!r}"
    )
    ...
    assert len(cmds) == 18, (
        f"generate_handler! in main.rs must register 18 commands (actual: "
        f"{len(cmds)}: {cmds})"
    )
    assert cmds[0] == "dispatch", (
        f"First command must be `dispatch` (actual: {cmds[0]!r})."
    )
    ...
    assert actual_typed == expected_typed, (
        f"Typed shortcuts drift. Expected {sorted(expected_typed)}, got "
        f"{sorted(actual_typed)}."
    )

    # Stale "ONE generic `dispatch`" must be gone.
    assert "ONE generic `dispatch`" not in body, (
        "stale 'ONE generic `dispatch`' phrase must be removed."
    )


def test_gp94_main_rs_line_count_is_264():
    """Doc claims 288 lines; main.rs must actually be 288 lines.
    ...
    """
    doc = _read(ARCH_DOC)
    assert "288 lines" in doc, "Doc must claim '288 lines' for main.rs."
    actual = sum(1 for _ in _read(MAIN_RS).splitlines())
    assert actual == 288, (
        f"src-tauri/src/main.rs must be 288 lines (actual: {actual}). "
        f"Update the doc + this test together."
    )
    # Stale 264 + 488 must NOT be in the doc.
...
def test_gp95_module_paths_use_package_form():
    ...
    assert len(crash_files) == 8, (
        f"crash_handler/ must be an 8-file package (actual: {len(crash_files)})."
    )
    level_files = list((ROOT / "voice_typer/server/level_monitor").glob("*.py"))
    assert len(level_files) == 5, (
        f"level_monitor/ must be a 5-file package (actual: {len(level_files)})."
    )
    cts_files = list((ROOT / "voice_typer/server/clipboard_target_safety").glob("*.py"))
    assert len(cts_files) == 4, (
        f"clipboard_target_safety/ must be a 4-file package (actual: {len(cts_files)})."
    )
...
def test_gp96_shutdown_controller_entry_points_match_code():
    ...
    for name in ["`quit()`", "`_do_cleanup()`", "`_do_fast_cleanup()`", "`_atexit_cleanup()`"]:
        assert name in doc, (
            f"shutdown_controller.md must document {name}."
        )
    # Stale entry-point names must be gone.
    assert "`shutdown()`" not in doc, (
        "stale `shutdown()` entry point must be removed "
        "(the primary entry point is `quit()`)."
    )
    assert "`force_shutdown()`" not in doc, (
        "stale `force_shutdown()` entry point must be removed "
        "(replaced by `_do_fast_cleanup()`)."
    )
...
def test_gp97_audio_quality_controller_entry_points_match_code():
    ...
    assert "## IPC Surface" not in doc, (
        "audio_quality_controller.md must NOT have an IPC Surface section "
        "(controller publishes no events)."
    )
...
def test_gp98_sidecar_ws_doc_is_accurate():
    ...
    assert "one-shot bearer-token auth" in doc, (
        "sidecar_ws.md must say 'one-shot bearer-token auth'."
    )
    assert "hmac.compare_digest" in doc, (
        "sidecar_ws.md must mention hmac.compare_digest (constant-time)."
    )
    assert "NOT an HMAC scheme" in doc, (
        "sidecar_ws.md must explicitly say 'NOT an HMAC scheme'."
    )
    # Stale phrasing must be gone.
    assert "HMAC auth handshake" not in doc, (
        "stale 'HMAC auth handshake' must be removed."
    )
    ...
    assert list(sig_run.parameters) == ["server"], (
        f"sidecar_ws.run signature must be (server) (actual: {sig_run})."
    )
    sig_emit = inspect.signature(sidecar_ws._emit_server_started)
    assert list(sig_emit.parameters) == ["port", "protocol"], (
        f"_emit_server_started signature must be (port, protocol=None) "
        f"(actual: {sig_emit})."
    )
...
def test_timer_coordinator_doc_matches_code():
    ...
    for method in ["_schedule_timer", "_cancel_pending_timers"]:
        assert callable(getattr(TimerCoordinator, method, None)), (
            f"TimerCoordinator must define `{method}`."
        )
...
def test_volume_controller_doc_matches_code():
    ...
    for method in ["_on_volume_crash_restore", "_duck_volume", "_restore_volume"]:
        assert callable(getattr(VolumeController, method, None)), (
            f"VolumeController must define `{method}`."
        )
...
def test_index_lists_all_five_module_docs():
    ...
    for name in [
        ...
        assert (ROOT / "docs" / "modules" / f"{name}.md").exists(), (
            f"docs/modules/{name}.md must exist."
        )
...
def test_error_envelope_contract_uses_transport_tcp_path():
    ...
    assert "ipc_server._handle_tcp_connection" not in doc, (
        "Stale 'ipc_server._handle_tcp_connection' must be replaced."
    )
