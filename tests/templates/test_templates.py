"""TemplateManager persistence tests split out of the former ``tests/test_history_and_models.py``.

Domain: templates — disk persistence via TemplateManager._save,
reload on restart, and the service-layer save/get round-trip +
...
``templates_dir`` fixture (tmp_path with _config_dir monkeypatched)
is provided by the top-level ``tests/conftest.py``.
"""
