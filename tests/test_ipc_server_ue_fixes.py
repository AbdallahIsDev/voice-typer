...
- UE-32 (Medium): ``_COMMAND_REGISTRY`` + ``_READONLY_COMMANDS`` +
                  ...
                  ``test_ec4_python_command_registry_parity``, etc.).
...
# ── UE-13: stdin listener gate behind VOICE_TYPER_ALLOW_STDIN_IPC ──────


class TestUE13StdinGate:
    ...
    def test_stdin_thread_none_when_gate_refuses(self, monkeypatch) -> None:
        ...
        server.start()
        try:
            # UE-13: the gate must prevent the stdin listener thread
...
# ── UE-18: _handle_shutdown re-entrancy gate + thread registry ─────────


class TestUE18ShutdownGate:
    ...
    def test_double_handle_shutdown_no_ops_second_invocation(self) -> None:
        ...
        while time.time() < deadline and server.service.quit.call_count < 1:
            time.sleep(0.005)
        # UE-18: service.quit is called EXACTLY ONCE — the second
...
# ── UE-32: registry extraction ─────────────────────────────────────────


class TestUE32RegistryExtraction:
    """UE-32 (Medium): ``_COMMAND_REGISTRY`` + ``_READONLY_COMMANDS`` +
    ``_PYTHON_ONLY_COMMANDS`` are canonical to
    :mod:`voice_typer.server.ipc.registry`.
    ...
    def test_registry_dict_same_keys_and_values_as_before(self) -> None:
        ...
        # test_ec4_python_command_registry_parity, test_ipc_command_registry_sync,
        ...
        # The IPC-1 reconciliation comment documents 63 commands.
