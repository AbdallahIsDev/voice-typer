...
Spec (§8.4):
  ...
  CRITICAL: the consent flag is ``runtime_pack_consent`` — NOT
  ``huggingface_consent`` — because the pack download phones home to
  GitHub Releases (revealing user IP to Microsoft).

Tested behaviors:

  1. ``config=None`` → ``PackConsentRequiredError`` raised.
  2. ``config.runtime_pack_consent=False`` → error raised.
  3. ``config.runtime_pack_consent=True`` → no error (download proceeds).
  4. The exception's ``consent_field`` is ``"runtime_pack_consent"``.
...
def _config(*, runtime_pack_consent: bool = False, huggingface_consent: bool = False):
    """Build a minimal config-like object with both consent flags."""
    return SimpleNamespace(
        runtime_pack_consent=runtime_pack_consent,
        huggingface_consent=huggingface_consent,
    )


class TestConsentGate:
    """§8.4 — pack download requires runtime_pack_consent."""

    def test_no_config_raises(self):
        with pytest.raises(pack.PackConsentRequiredError):
            pack.require_runtime_pack_consent(None, version="v1")

    def test_consent_false_raises(self):
        with pytest.raises(pack.PackConsentRequiredError):
            pack.require_runtime_pack_consent(_config(runtime_pack_consent=False), version="v1")

    def test_consent_true_passes(self):
        # Should not raise.
        pack.require_runtime_pack_consent(_config(runtime_pack_consent=True), version="v1")

    def test_consent_field_is_runtime_pack_consent(self):
        """The exception's structured field points at the right Settings toggle."""
        with pytest.raises(pack.PackConsentRequiredError) as exc_info:
            pack.require_runtime_pack_consent(_config(), version="v1")
        assert exc_info.value.consent_field == "runtime_pack_consent"
    ...
    def test_huggingface_consent_alone_does_not_authorize(self):
        ...
        """
        with pytest.raises(pack.PackConsentRequiredError):
            pack.require_runtime_pack_consent(
                _config(runtime_pack_consent=False, huggingface_consent=True),
                version="v1",
            )

    def test_version_recorded_on_exception(self):
        with pytest.raises(pack.PackConsentRequiredError) as exc_info:
            pack.require_runtime_pack_consent(_config(), version="v2.3.1")
