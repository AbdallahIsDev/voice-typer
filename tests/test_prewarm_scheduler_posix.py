...
# plist XML escaping via ElementTree ──────────────────
...
# Linux register also starts the timer ──────────────────
...
# unregister stops the SERVICE unit too ─────────────────
...
# ExecStart uses autostart_launcher (not bare ipc_server) ────


class TestLinuxAppServiceExecStartUsesLauncher:
    """FR-10: ``_build_linux_app_service`` must emit an ``ExecStart`` that
    points at ``voice_typer.server.autostart_launcher --hidden`` (the same
    entry point the OS autostart uses) rather than the bare
    ``voice_typer.server.ipc_server`` backend.

    The bare IPC server is backend-only — no tray, no Tauri/Electron
    frontend, no UI surface. If a user manually ``enable``d the unit
    under the old ExecStart, they got a supervised process with no
    user-visible surface. The launcher orchestrates BOTH the frontend
    and backend (and ``--hidden`` matches the .desktop autostart path).
    """

    def test_execstart_references_autostart_launcher(self):
        """ExecStart must reference ``autostart_launcher`` (not just
        ``ipc_server``)."""
        service = psp._build_linux_app_service()
        assert "voice_typer.server.autostart_launcher" in service, (
            f"FR-10 regression: ExecStart does not reference autostart_launcher; got: {service!r}"
        )

    def test_execstart_does_not_use_bare_ipc_server(self):
        """The bare ``-m voice_typer.server.ipc_server`` (with no
        launcher) must NOT appear in ExecStart. The launcher imports
        the IPC server internally, but the unit's ExecStart must invoke
        the launcher, not the backend directly."""
        service = psp._build_linux_app_service()
        # Extract the ExecStart line(s) only — we don't want to false-
        # positive on a docstring/comment mentioning ipc_server.
        exec_start_lines = [line for line in service.splitlines() if line.startswith("ExecStart=")]
        assert exec_start_lines, "no ExecStart= line found in service unit"
        for line in exec_start_lines:
            assert "voice_typer.server.ipc_server" not in line, (
                f"FR-10 regression: ExecStart still uses bare ipc_server: {line!r}"
            )

    def test_execstart_includes_hidden_flag(self):
        """``--hidden`` must be present so the tray starts minimized
        (matching the .desktop autostart entry's behaviour)."""
        service = psp._build_linux_app_service()
        exec_start_lines = [line for line in service.splitlines() if line.startswith("ExecStart=")]
        assert exec_start_lines, "no ExecStart= line found in service unit"
        assert any("--hidden" in line for line in exec_start_lines), (
            f"FR-10 regression: ExecStart missing --hidden flag; got: {exec_start_lines!r}"
        )

    def test_service_still_has_restart_and_type_simple(self):
        """FR-10 must not regress the existing PLAT-019 guarantees:
        ``Restart=on-failure`` and ``Type=simple`` must still be
        present."""
        service = psp._build_linux_app_service()
        assert "Restart=on-failure" in service
        assert "Type=simple" in service

    def test_watchdog_sec_is_omitted(self):
        """sd_notify is a future enhancement (documented in the
        docstring). ``WatchdogSec=`` must NOT be present in the unit —
        adding it without sd_notify support would cause systemd to mark
        the unit failed after the first watchdog interval."""
        service = psp._build_linux_app_service()
        assert "WatchdogSec=" not in service, (
            f"WatchdogSec= present but launcher does not yet implement sd_notify; "
            f"systemd would fail the unit. Got: {service!r}"
        )
