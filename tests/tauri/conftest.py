...
# ── Binary-stub loss guard ─────────────────────────────────────────────
# The Tauri build needs placeholder binaries under ``src-tauri/bin/`` +
# ``src-tauri/resources/native/`` (generated by
# ``scripts/gen_tauri_icons_stub.py``, gitignored). The stub test module
# (``test_gen_tauri_icons_stub.py``) deletes + regenerates them per test,
# and its fixture restores the pre-test state — but a worker killed
# between ``--clean`` and regenerate (or a transient Windows write
# failure inside ``generate``) still leaves the tree stub-less, and every
# later ``cargo check`` then fails with "resource path ... doesn't
# exist". This session net closes that gap: snapshot the pre-session
# stub set at start; at finish, re-create anything that went missing.
# Best-effort by design (it must never fail the suite) — the fixture's
# loud ``pytest.fail`` remains the primary signal; this is the net.
_STUB_TRIPLES = (
...
def _canonical_stub_paths() -> list[str]:
    ...
    Mirrors the generator's triple table (see SIDECAR_TRIPLES there and
    ``_stub_paths()`` in ``test_gen_tauri_icons_stub.py``). Kept as data
    (not an import) so this hook never imports test modules.
    """
...
def pytest_sessionfinish(session, exitstatus):
    ...
    Controller-only (xdist workers report via ``workerinput`` and skip —
    the controller runs this once after all workers finish). Best-effort:
    warns, never raises, so a broken restore can never fail the suite.
    """
    try:
