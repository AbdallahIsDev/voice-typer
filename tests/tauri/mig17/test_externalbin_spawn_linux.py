...
5. The crash supervisor (in ``src-tauri/src/sidecar/supervisor.rs``, backed
...
def test_tauri_conf_shell_config_is_v2_valid(tauri_conf) -> None:
    ...
    launches the app). This is the same on Linux as on Windows +
    macOS — Tauri's shell plugin is cross-platform. Sidecar spawning
    is owned by the Rust host via ``app.shell().sidecar(...)``
    (not ACL-gated); the JS-facing ``shell:allow-spawn`` grant keeps
...
# ─── Test 6: crash supervisor (backoff 500ms → 8s, cap 5) ────────
...
def test_supervisor_backoff_constants(util_rs_source) -> None:
    ...
    is no per-platform supervisor config.
    """
...
def test_supervisor_backoff_schedule_length_matches_cap(util_rs_source) -> None:
    ...
    assert len(steps) == cap, (
        f"SUPERVISOR_BACKOFF_MS.len() ({len(steps)}) must equal "
        f"SUPERVISOR_MAX_RETRIES ({cap}) so the loop iterates exactly N times"
    )
...
def test_supervisor_respawn_calls_spawn_sidecar(supervisor_rs_source) -> None:
    ...
    on Linux as on Windows + macOS — the supervisor is
    platform-agnostic.
    """
