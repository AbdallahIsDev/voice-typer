...
SPEC_PATH = PROJECT_ROOT / "scripts" / "build" / "lausu.spec"
...
def spec_text() -> str:
    ...
    assert SPEC_PATH.is_file(), (
        f"lausu.spec not found at {SPEC_PATH}. "
...
def test_spec_file_exists():
    ...
    assert SPEC_PATH.stat().st_size > 1000, (
        f"lausu.spec is suspiciously small ({SPEC_PATH.stat().st_size} bytes); "
...
def test_spec_docstring_documents_fallback_role(spec_text: str):
    ...
    assert "PyInstaller fallback" in spec_text, (
        "lausu.spec docstring must mention 'PyInstaller fallback' (ADR-0020 §4.5 framing)."
    )
    assert "ADR-0020" in spec_text, "lausu.spec docstring must reference ADR-0020 §4.5."
    assert "Nuitka" in spec_text, (
        "lausu.spec docstring must name Nuitka as the primary path (PyInstaller is the safety net, not the default)."
    )
...
def test_spec_targets_ipc_server_entry_point(spec_text: str):
    ...
    assert "ipc_server.py" in spec_text, (
        "lausu.spec must reference ipc_server.py as the entry point "
    ...
    assert (
        ...
        or "voice_typer/server/ipc_server.py" in spec_text
    ), "lausu.spec must target voice_typer/server/ipc_server.py (the canonical sidecar entry point)."
...
def test_spec_includes_ctranslate2_hidden_import(spec_text: str):
    ...
    assert '"ctranslate2"' in spec_text or "'ctranslate2'" in spec_text, (
        "lausu.spec _hiddenimports must include 'ctranslate2' "
...
def test_spec_includes_faster_whisper_hidden_import(spec_text: str):
    ...
    assert '"faster_whisper"' in spec_text or "'faster_whisper'" in spec_text, (
        "lausu.spec _hiddenimports must include 'faster_whisper' "
...
def test_spec_uses_onefile_mode_not_onedir(spec_text: str):
    ...
    assert "COLLECT(" not in spec_text, (
        "lausu.spec must NOT use COLLECT(), that's onedir mode. "
        "Tauri externalBin requires a single executable per triple "
        "(onefile). See GAP-2."
    )
    assert "EXE(" in spec_text, "lausu.spec must define an EXE() call."
    assert "a.binaries" in spec_text, "lausu.spec EXE() must wrap a.binaries (onefile signature)."
    assert "a.datas" in spec_text, "lausu.spec EXE() must wrap a.datas (onefile signature)."
    # The spec docstring / comments should self-document onefile mode.
    assert "onefile" in spec_text.lower(), (
        "lausu.spec should mention 'onefile' in its docstring or comments (build mode self-documentation)."
    )
...
def test_spec_reads_tauri_sidecar_env_var(spec_text: str):
    ...
    assert "VOICE_TYPER_TAURI_SIDECAR" in spec_text, (
        "lausu.spec must read VOICE_TYPER_TAURI_SIDECAR env var (ADR-0020 §4.5 Tauri sidecar mode switch)."
    )
    ...
    assert re.search(r'os\.environ\.get\s*\(\s*["\']VOICE_TYPER_TAURI_SIDECAR["\']', spec_text) is not None, (
        "lausu.spec must read VOICE_TYPER_TAURI_SIDECAR via os.environ.get(...) (idiomatic env var read)."
    )
...
def test_spec_tauri_mode_enables_console_for_ws_handshake(spec_text: str):
    ...
    assert "_CONSOLE = True" in spec_text, (
        "lausu.spec must set _CONSOLE = True in the Tauri sidecar "
    ...
    assert "server_started" in spec_text, (
        "lausu.spec docstring must mention server_started JSON "
...
def test_spec_produces_per_triple_output_filename(spec_text: str):
    ...
    assert 'f"python-sidecar-{_TRIPLE}"' in spec_text or "python-sidecar-{_TRIPLE}" in spec_text, (
        "lausu.spec must construct the exe name as "
...
def test_spec_triple_construction_matches_rust_target_triples(spec_text: str):
    ...
    assert "pc-windows-msvc" in spec_text, (
        "lausu.spec must construct the Windows Rust target triple (<arch>-pc-windows-msvc)."
    )
    # macOS triple.
    assert "apple-darwin" in spec_text, "lausu.spec must construct the macOS Rust target triple (<arch>-apple-darwin)."
    # Linux triple.
    assert "unknown-linux-gnu" in spec_text, (
        "lausu.spec must construct the Linux Rust target triple (<arch>-unknown-linux-gnu)."
    )
...
def test_spec_bundles_ct2_native_libs_via_hidden_import(spec_text: str):
    ...
    assert '"ctranslate2"' in spec_text or "'ctranslate2'" in spec_text, (
        "lausu.spec must list 'ctranslate2' as a hidden import so "
    ...
    assert "ctranslate2" not in excludes_block, (
        "lausu.spec must NOT list 'ctranslate2' in excludes=, that would strip the CT2 native libs from the bundle."
    )
...
def test_spec_does_not_exclude_faster_whisper(spec_text: str):
    ...
    assert "faster_whisper" not in excludes_block, (
        "lausu.spec must NOT list 'faster_whisper' in excludes=, that would strip the ASR engine from the bundle."
    )
