...
PLATFORM_TEMPDIR_PATTERNS: dict[str, list[re.Pattern[str]]] = {
    "windows": [
        # {CACHE_DIR}/lausu/onefile-tmp, the Nuitka-documented token
        re.compile(r"\{CACHE_DIR\}[/\\]+lausu[/\\]+onefile-tmp"),
        # {CACHE_DIR} (see the mig15 pin).
        re.compile(r"%LOCALAPPDATA%[/\\]+lausu[/\\]+onefile-tmp"),
        # $XDG_CACHE_HOME/lausu/onefile-tmp (rare on Windows but allowed)
        re.compile(r"\$XDG_CACHE_HOME/lausu/onefile-tmp"),
    ],
    "macos": [
        # ~/Library/Caches/lausu/onefile-tmp
        re.compile(r"~/Library/Caches/lausu/onefile-tmp"),
        # $HOME/Library/Caches/lausu/onefile-tmp
        re.compile(r"\$HOME/Library/Caches/lausu/onefile-tmp"),
        # $XDG_CACHE_HOME/lausu/onefile-tmp (XDG override)
        re.compile(r"\$XDG_CACHE_HOME/lausu/onefile-tmp"),
    ],
    "linux": [
        # $XDG_CACHE_HOME/lausu/onefile-tmp
        re.compile(r"\$XDG_CACHE_HOME/lausu/onefile-tmp"),
        # $HOME/.cache/lausu/onefile-tmp
        re.compile(r"\$HOME/\.cache/lausu/onefile-tmp"),
        # ~/.cache/lausu/onefile-tmp
        re.compile(r"~/\.cache/lausu/onefile-tmp"),
        # ${XDG_CACHE_HOME:-$HOME/.cache}/lausu/onefile-tmp (bash default-expansion)
        re.compile(r"\$\{XDG_CACHE_HOME:-\$HOME/\.cache\}/lausu/onefile-tmp"),
    ],
}
...
def test_windows_tempdir_spec_per_user_cache_dir(script_texts: dict[str, str]):
    """Windows tempdir-spec must be ``%LOCALAPPDATA%/lausu/onefile-tmp``."""
...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "`$HOME/Library/Application Support/lausu/onefile-tmp` but the "
        "MIG-1.8 task spec requires `~/Library/Caches/lausu/onefile-tmp` "
        "(or `$XDG_CACHE_HOME/lausu/onefile-tmp`). See "
...
def test_macos_tempdir_spec_per_user_cache_dir(script_texts: dict[str, str]):
    """macOS tempdir-spec must be ``~/Library/Caches/lausu/onefile-tmp``."""
...
def test_linux_tempdir_spec_per_user_cache_dir(script_texts: dict[str, str]):
    """Linux tempdir-spec must be ``$XDG_CACHE_HOME/lausu/onefile-tmp``."""
...
def test_tempdir_spec_includes_voice_tyer(script_texts: dict[str, str], platform: str):
    """The tempdir-spec must include ``lausu`` in the path."""
    text = script_texts[platform]
    spec = _extract_resolved_tempdir_spec(text)
    assert spec is not None, f"build_sidecar_{platform}.sh: could not extract --onefile-tempdir-spec value."
    assert "lausu" in spec, (
        f"build_sidecar_{platform}.sh --onefile-tempdir-spec value `{spec}` "
        "must include `lausu` in the path to avoid collision with "
