...
existing ``TestDE3MigratorFailureDoesNotBumpSchemaVersion`` tests
...
``tests/test_config_de_fixes.py`` pin the same behaviour from a
...
# ── XZ-14-16: schema_version must NOT bump on migrator failure ─────────────


class TestXZ1416MigratorFailureDoesNotBumpSchemaVersion:
...
# ── XZ-14-16: failed-migration loop must NOT continue to later migrators ────


class TestXZ1416FailureBreaksMigrationLoop:
...
# ── XZ-14-16: timestamped .bak file with failed-version in filename ────────


class TestXZ1416FailedMigrationBackup:
    """XZ-14-16: on migrator failure a ``.bak`` file must be saved with
    a UTC timestamp and the failed target version in the filename, so
    ...
    def test_bak_filename_has_timestamp_and_failed_version(
        ...
        assert self._BAK_RE.match(name), (
            ...
            f"'config.json.bak.failed-migration-YYYYMMDD-HHMMSS-to-v<N>'."
        )
...
# ── XZ-14-16: failure must be loudly reported in logs ──────────────────────


class TestXZ1416FailureIsLoggedAtError:
    ...
    def test_error_log_names_failed_version_and_exception_type(
        ...
        # exception type was raised.  The DE-3 tests pin the substring
    ...
    def test_load_does_not_raise_on_migrator_failure(
        ...
        tests in ``tests/test_config_de_fixes.py`` rely on this
