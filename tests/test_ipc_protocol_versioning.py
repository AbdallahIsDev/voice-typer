...
def test_source_contains_protocol_version_check_before_token_check() -> None:
    ...
    assert "protocol_version" in src, (
        "DR-21: _handle_tcp_connection must reference 'protocol_version' in its auth-handshake block."
    )
    assert "IPC_PROTOCOL_VERSION" in src, (
        "DR-21: _handle_tcp_connection must reference the IPC_PROTOCOL_VERSION constant."
    )
...
def test_auth_rejects_frame_with_mismatched_protocol_version() -> None:
    ...
    assert decoded["type"] == "error", f"Expected error envelope, got type={decoded.get('type')!r}"
    data_obj = decoded["data"]
    assert data_obj["code"] == PROTOCOL_VERSION_MISMATCH_CODE, (
        f"Expected code={PROTOCOL_VERSION_MISMATCH_CODE!r}, got {data_obj.get('code')!r}"
    )
