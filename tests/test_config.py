...
class TestDeadAirTimeoutConfig:
    """AUDIO-DEAD: Tests for the dead_air_timeout config field and IPC allowlist."""

    def test_default_dead_air_timeout(self):
        """Default dead_air_timeout should be 30.0 seconds."""
        c = Config()
        assert c.dead_air_timeout == 30.0

    @pytest.mark.parametrize("value", [0.0, 10.0, 30.0, 60.0, 300.0])
    def test_dead_air_timeout_roundtrip(self, tmp_path, monkeypatch, value):
        """dead_air_timeout should survive save/load roundtrip."""
        monkeypatch.setattr("voice_typer.server.config._config_dir", lambda: tmp_path)
        c = Config(dead_air_timeout=value)
        c.save()
        loaded = Config.load()
        assert loaded.dead_air_timeout == value

    def test_dead_air_timeout_in_ipc_allowlist(self):
        """dead_air_timeout should be in IPC_CONFIG_ALLOWLIST."""
        from voice_typer.server.config import IPC_CONFIG_ALLOWLIST
        assert "dead_air_timeout" in IPC_CONFIG_ALLOWLIST

    def test_dead_air_timeout_ipc_validation_accepts_valid(self):
        """IPC validation should accept valid dead_air_timeout values."""
        from voice_typer.server.config import validate_config_update
        validated, errors = validate_config_update({"dead_air_timeout": 45.0})
        assert not errors
        assert validated["dead_air_timeout"] == 45.0

    def test_dead_air_timeout_ipc_validation_rejects_negative(self):
        """IPC validation should reject negative dead_air_timeout values."""
        from voice_typer.server.config import validate_config_update
        validated, errors = validate_config_update({"dead_air_timeout": -1.0})
        assert errors  # should have validation errors

    def test_dead_air_timeout_ipc_validation_rejects_too_large(self):
        """IPC validation should reject dead_air_timeout > 600."""
        from voice_typer.server.config import validate_config_update
        validated, errors = validate_config_update({"dead_air_timeout": 601.0})
        assert errors

    def test_dead_air_timeout_ipc_validation_rejects_wrong_type(self):
        """IPC validation should reject non-float dead_air_timeout values."""
        from voice_typer.server.config import validate_config_update
        validated, errors = validate_config_update({"dead_air_timeout": "thirty"})
        assert errors

    def test_dead_air_timeout_zero_accepted(self):
        """IPC validation should accept 0 (disabled)."""
        from voice_typer.server.config import validate_config_update
        validated, errors = validate_config_update({"dead_air_timeout": 0.0})
        assert not errors
        assert validated["dead_air_timeout"] == 0.0
