...
**Status:** ⚠️ Partial — purge_user_data() function added to voice_typer/server/config.py (data-layer helper enumerating all user-data files/dirs); uninstall-script wiring (scripts/linux/*, src-tauri/tauri.*.conf.json, NSIS, macOS helper) owned by other agents
...
- **Severity**: Critical
- **Status**: ⚠️ Partial — dead-code ipc/server.py already deleted per CR-019; ipc_server.py retains canonical implementation; ipc/__init__.py re-exports from leaf submodules; further consolidation (reducing ipc_server.py to shim) blocked by 164 inspect.getsource tests + 3218-LOC file
...
- **Severity**: Critical
- **Status**: ❌ Not Fixed — too large for 10-min sub-agent ceiling (multi-hour/day refactor) — recorder.py is 4080 LOC; proposed decomposition (DeviceResolver, DeviceHealthMonitor, VadController, AudioWorker, EventWorker, ChunkProcessor) is a multi-hour refactor; partial decomposition already done via _recorder_split.py + device_manager.py
...
- **Severity**: High
- **Status**: ❌ Not Fixed — too large for 10-min sub-agent ceiling (multi-hour/day refactor) — app.py is 950 LOC (already trimmed from 1179); extracting additional controllers (RepasteController, UndoController, RestartController) is a multi-hour refactor with monkeypatch-test breakage risk
...
- **Severity**: High
- **Status**: ❌ Not Fixed — cross-file coordination required — proposed 2-pronged fix requires (a) persisting raw audio chunks in voice_typer/server/recording/recorder.py (Agent 14 scope) + (b) emitting dictation_lost from Rust supervisor (would be false-positive on every disconnect — needs Python recorder state propagation)
...
## [DE-13] — recording_controller: audio buffer persists in memory 30+ min after user-initiated cancel — privacy violation
...
**Description:** In `recording_controller.py` (lines 505-520 transcription thread + `_force_recover_from_stuck_transcription:798-853`), when the transcription thread is stuck (ctranslate2 deadlock), the watchdog fires at 90s × 3 = 270s and calls `_force_recover_from_stuck_transcription`, which marks the cycle as cancelled in `_cancelled_cycle_ids` (so late results aren't pasted), resets `_busy_event` and tray state, and calls `_stop_watchdog_thread()`. But it does NOT clear the `audio` bytes reference held by the still-running transcription thread (the `audio` local in `transcribe_thread()`'s closure, line 508). The stuck ctranslate2 call holds the audio bytes for as long as it runs — the docstring at line 92-93 acknowledges '5-30 min later.' During that window, the raw audio of the user's voice is recoverable from process memory by any debugger or memory dump. For a privacy-sensitive app processing biometric data (per NEW-PRIV-009), this is a data-retention violation.

**Root Cause:** Verified: force-recovery cancels the cycle logically but does not release the audio bytes reference held by the stuck ctranslate2 call.

**Progress:** None yet.

**Related Files:**
- `voice_typer/server/recording_controller.py`
**Fix:** Restructure `transcribe_thread` to read audio from a shared, clearable slot (e.g., `self._current_audio`) rather than a closed-over local. After force-recovery, set `self._current_audio = None` so the audio bytes can be GC'd. At minimum, document the retention window in the privacy policy. Add a test that simulates force-recovery and verifies the audio reference is cleared.
...
**Status:** ⚠️ Partial — ErrorCodes/LegacyErrorCodes classes added as single source of truth with contract test; 35+ emitter sites still use bare string literals
...
**Status:** ⚠️ Partial — Rust logs/ dir cleanup + prewarm.log + docs done; Electron-side deleteAllPersonalData IPC handler NOT implemented
...
### Findings from Session 1 (DT prefix)

## DT-1 — useHistoryCache/useHistoryExport are empty stubs (History page broken)
**Status:** ❌ Not Fixed — useHistoryCache/useHistoryExport are still empty stubs (`load = async () => {}`), no IPC wiring
**Description:** The History page's data hooks (`pages/history/hooks/useHistoryCache.ts` 57 lines and `useHistoryExport.ts` 23 lines) are empty no-op stubs. `load = useCallback(async () => {}, [])`, `loadMore`/`refreshFromEvent`/`setFilter` are all no-ops. The page's `useEffect(() => { load(); }, [load])` runs once but `load()` does nothing — `records` stays `[]`, so the EmptyState "no transcriptions" branch always renders. Sibling hooks (`useTemplates.ts` 304 lines, `useVocabulary.ts` 288 lines) have real IPC wiring; these two were never ported after the BG-54 split.
**Root Cause:** BG-54 refactor extracted the hook contracts but never ported the IPC logic from the pre-split monolith.
**Impact:** History page is non-functional: list never loads, search/loadMore/export are no-ops. Stats display always shows "0 / 0 chars today".
**Progress:** None yet.
**Related Files:**
...
**Fix:** Port the actual logic into useHistoryCache (model on useVocabulary pattern: wire `load` to `call<HistoryRecord[]>("get_history"/"get_favorites"/"search_history")`, set loading/loadError/hasMore state, expose setRecords/setStats/setHasMore). Port useHistoryExport.doExport to the filter-aware paging loop. Verify History.test.tsx passes.
**Severity:** 🔴 Critical

## DT-8 — Dead `paste_text` command (~700 LOC retained for DevTools debugging)
**Status:** ❌ Not Fixed — dead paste_text NOT behind #[cfg(feature = "dev-paste-debug")], fully compiled in release
**Description:** `sidecar_cmds.rs:613-638` defines `paste_text` as a `#[tauri::command]` retained "for DevTools debugging." Grep confirms: no Python code publishes a `paste_text` event, no TS code invokes `invoke('paste_text', ...)`. The Python sidecar does its own paste internally in `dictation_pipeline.py:990-1010`. The 622-line `paste.rs` implementation is also dead in production.
**Root Cause:** ADR-0020 §6.2 design called for the React UI to invoke this, but Python-side paste became the permanent architecture.
**Impact:** ~700 LOC of dead production code that must be kept in sync with the live Python paste path.
**Progress:** None yet.
**Related Files:**
- `src-tauri/src/commands/sidecar_cmds.rs:613-638`
- `src-tauri/src/commands/paste.rs` (622 lines)**Fix:** Move `paste_text` + `paste.rs` behind `#[cfg(feature = "dev-paste-debug")]` so the code is excluded from release builds. Update migration glue tests that source-grep the signature.
**Severity:** 🟢 Low

## DT-12 — 75 `getattr(config, "field", DEFAULT)` dead-defensive calls
**Status:** ❌ Not Fixed — 28 files with ~93 call sites still using getattr(config, "field", DEFAULT) pattern
**Description:** 75 sites across `voice_typer/server/` use `getattr(config, "field_name", DEFAULT)` where `Config` is a non-frozen `@dataclass` that ALWAYS provides the field. 26 of these have a literal numeric default that duplicates the Config dataclass default. Examples: `audio_chain_builder.py:44 getattr(config, "noise_filter_notch_frequency_hz", 0.0)`, `recorder.py:514 getattr(config, "sample_rate", 16000)`, `service/status.py:121 getattr(app.config, "sample_rate", 16000)`.
**Root Cause:** Dead-defensive code; the DEFAULT fallback can never fire on a real Config instance.
**Impact:** If Config.sample_rate is bumped from 16000 to 24000, the 3 fallback sites that hardcode 16000 will return wrong values when the fallback fires (e.g. a MagicMock test config without sample_rate set).
**Progress:** None yet.
**Related Files:**
- `voice_typer/server/audio_chain_builder.py`
- `voice_typer/server/recording/recorder.py`
...
## DT-22 — ipc_server.py (3095 lines) — Critical spaghetti monolith
**Status:** ❌ Not Fixed — ipc_server.py (3137 lines) NOT split; no ipc/transport_tcp.py, heartbeat.py, dispatcher.py, sender.py
**Description:** `voice_typer/server/ipc_server.py` is 3095 lines. `IPCServer` class (line 366) mixes TCP transport, stdin transport, heartbeat subsystem, dispatch routing, command registry, push/send, and 6 resident handlers. `_handle_tcp_connection` is 382 lines. `_send` is 359 lines. `main()` is 259 lines. `_shutting_down_error` is 218 lines. `__init__` is 199 lines.
**Root Cause:** ARCH-REFAC-002 extracted handler mixins but stopped at the transport/dispatch boundary.
**Impact:** Adding a new transport requires touching the god-class; heartbeat bugs require reading 3095 lines.
**Progress:** None yet.
**Related Files:**
- `voice_typer/server/ipc_server.py`**Fix:** Extract `ipc/transport_tcp.py` (TCP accept + connection handler), `ipc/transport_stdio.py` (stdin loop), `ipc/heartbeat.py` (heartbeat loop + timeout), `ipc/dispatcher.py` (_dispatch + shutting_down_error), `ipc/sender.py` (_send + push). Keep `IPCServer` as the mixin composer + `__init__` + thin facades. Target ≤800 lines.
**Severity:** 🔴 Critical

## DT-23 — history_db.py (2743 lines) — Critical spaghetti monolith
**Status:** ❌ Not Fixed — history_db.py (2743 lines) NOT split; no history_db/schema.py, writer.py, connections.py, retention.py, queries.py
**Description:** `voice_typer/server/history_db.py` is 2743 lines. `HistoryDB` class (line 368) mixes schema + migration, single-writer-thread engine, connection pool, WAL checkpoint/retention scheduler, and 11 CRUD/query methods. `_init_db_schema` is 234 lines. `apply_retention` is 136 lines. `schedule_periodic_retention` is 157 lines.
**Root Cause:** SQLite persistence concerns and domain concerns mixed in one class.
**Impact:** Schema migration code co-resides with hot-path CRUD; retention sweep logic co-resides with simple reads.
**Progress:** None yet.
**Related Files:**
- `voice_typer/server/history_db.py`
**Fix:** Split into `history_db/schema.py` (init + migrations + corruption recovery), `history_db/writer.py` (writer loop + drain helpers), `history_db/connections.py` (pool), `history_db/retention.py` (retention scheduler), `history_db/queries.py` (read functions). Keep `history_db.py` as the public CRUD surface delegating to the writer. Re-export `HistoryDB`, `HistoryDBError` from `__init__.py`.
**Severity:** 🔴 Critical
