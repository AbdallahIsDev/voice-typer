...
### CI-ALL — Fix all errors and warnings from `CI-errors.md`

**Status:** ❌ Not Fixed — CI test failures are auto-documented in **`CI-errors.md`** (repo root, generated by the `ci-errors-doc` workflow job from the latest GitHub Actions run). Fix everything listed there. No GitHub CLI or Actions-log access needed — read `CI-errors.md` directly.

**Description:** The `ci-errors-doc` job in `.github/workflows/build.yml` aggregates every test matrix leg's JUnit failures into `CI-errors.md` and commits it back. The cloud agent's job: open `CI-errors.md`, fix every failing/errored test at root cause, run the focused test to confirm green, then run the full suite. If `CI-errors.md` is empty (or says "No test failures"), CI is green — nothing to fix here.

**Current failures (from the last captured run — also present in `CI-errors.md`):**

1. **`TestRecorderCallbackWithAudioProcessor.test_callback_does_not_raise_with_processor`** — Expected 5 buffered chunks, got 4. File: `tests/` (recorder split regression).
2. **`test_microphone_watcher_coreaudio.test_coreaudio_watcher_start_raises_on_non_macos`** — Regex `'only available on macOS'` doesn't match new message. File: `tests/test_microphone_watcher_coreaudio.py:135`.
3. **`TestSidecarOwnership.test_native_hotkeys_module_defines_macos_backend`** — `base.py` is a facade re-exporting from `._core`; test inspects source text for `class SubprocessHotkeyBackend`. File: `voice_typer/server/native_hotkeys/base.py`.
4. **`TestShutdownConstants.test_shutdown_poll_interval_constant_removed`** — `SHUTDOWN_POLL_INTERVAL_MS` still in `src-tauri/src/util.rs` but test asserts removed.
5. **`TestConcurrentRestoreSerialization.test_two_concurrent_restores_do_not_overlap`** — Expected 2 restore() calls, got 3. File: `tests/` (clipboard restore serialization race).
6. **`TestSha256ByArchSync.test_host_arch_entry_updated_in_lockstep_with_flat`** — non-built arch (x86_64) has stale `'old'` value; must be empty.
7. **`TestLogCleanliness.test_happy_path_flows_are_log_clean`** — WARNING from `microphone_watcher_coreaudio.py:395` during happy path.
8. **`TestPublishTrayStateThreadSafe.test_concurrent_publishes_no_duplicate_emit`** — concurrent publishes with same state emitted twice (TOCTOU in `_last_published`).

**General CI pipeline issues (GP-*):** GP-66 (macOS CI hard-fails on missing binary instead of SKIP); GP-70 (missing `codesign --verify` step); GP-65 (`--sign` — already fixed); `upload-artifact@v5`/`setup-uv@v6` on Node 20 (bump to Node 24 majors).

**User Impact:** Red CI blocks merges and masks real regressions; unsigned binaries ship to SmartScreen; Node 20 deprecation will hard-fail when GitHub drops it.

**Root Cause:** Test regressions from recent refactors (recorder split, native hotkeys split, util.rs constant removal, clipboard restore serialization, SHA-256 by-arch sync) + stale test assertions.

**Progress:** Partial — failures documented; automation (`ci-errors-doc` job + `scripts/ci/write_ci_errors.py`) added so future failures are auto-captured.

**Related Files:**
- `CI-errors.md` (auto-generated — the source of truth for this task)
- `.github/workflows/build.yml` (ci-errors-doc job, test job upload step)
- `scripts/ci/write_ci_errors.py`
- `voice_typer/server/recording/`, `tests/`, `src-tauri/src/util.rs`, `voice_typer/server/native_hotkeys/base.py`, `voice_typer/server/microphone_watcher_coreaudio.py`

**Fix:** 1) Open `CI-errors.md`. 2) Fix each listed failure at root cause. 3) Confirm focused test green. 4) Run full pytest + vitest suite green. 5) Bump Node 20 actions to Node 24. 6) Fix GP-66, GP-70. 7) Trigger a CI run and confirm `CI-errors.md` is rewritten to "No test failures".
...
**Status:** ❌ Not Fixed — multiple GitHub Actions workflows fail across Electron and Tauri build pipelines. Gaps include outdated action versions (Node 20 deprecation), missing signing gates, hard-failing binary-existence checks instead of graceful skips, and orphaned configuration drift between per-platform workflow files.
