...
**Enrichment (2026-09-04 BP session — Wave 5):** A THIRD blocking site beyond the two filed here: the phase-6 `sync_autostart` → `is_autostart_enabled` → `schtasks /Query` runs SYNCHRONOUSLY on the startup critical path BEFORE hotkey registration (30 s hang ceiling; task_scheduler.py:57-60's docstring falsely claims a background thread), plus a no-op prewarm-sync daemon thread and a misdescribing INFO log every boot — filed as BP-129.
...
**Enrichment (2026-09-04 BP session — Wave 1):** Four new pieces of evidence: (1) `bench_startup.py:437-459` prints the §3.4 verdict (first_run ≤ 600 ms: FAIL) but `main()` returns 0 unconditionally — the target is never a gate; (2) the perf-ratchet baseline for the same metric sits at 913.4 ms, so BOTH layers pass while the product budget is missed; (3) audio-filter baselines use `tolterance_pct: 100` (fail only above 2× the baseline) while their own notes cite ADR-0009 budgets of <32 ms — the ratchet passes with the budget blown; (4) `bench_audio_filter_chain.py` reproduces non-finite output (see BP-57) 3/3 runs and still prints PASS — no `np.isfinite` assertion exists. Coverage gap list: the Tauri WS spine (frame encode path, `_safe_send` cap/timeout, outbound-queue drop-oldest, pipelined dispatch), TCP `_send` path, dispatcher, and history_db read/write latency are unbenched (bench_ipc covers validation/event_bus/rate-limiter only).
...
**Enrichment (2026-09-04 BP session — Wave 1):** One more concrete instance inside the recording collaborator cluster: `recording/audio_pipeline.py:574,904-906` threads raw RMS between `process_audio_chunk` and `run_vad_state_machine` via a transient instance attribute `_pending_raw_chunk_rms` (documented as needed because the callee "has a fixed positional signature"), and `:941` takes a second `_lock` acquisition per chunk that could merge with the append critical section.
...
**Enrichment (2026-09-04 BP session — Wave 1):** Two new god-method instances beyond the originally-cited sites: (1) `service/model/_downloads.py:372-839` — `_download_whisper_family` is a ~470-line method with four nesting levels and ten distinct concerns (single-flight, consent, cache probe, segmented lane, two nested retry fallbacks, verify, notify); (2) renderer page roots/hooks are the same family at 2-3× App.tsx's size — filed separately as BP-66 (Home 616 code lines, Settings 604, Onboarding 505, History 459; useTheme 393, useMicrophoneTestSession 471, useConnection 360, useOnboardingWizard 423).
...
**Enrichment (2026-09-04 BP session — Wave 1):** Verified post-rule bridge call sites in the recording package: `recording/audio_pipeline.py:62,665` and `recording/resampling.py:44,422,489` and `recording/_recorder_split.py:777,877` still call production paths through `_recording_pkg._get_resample_poly()` / `_recording_pkg._secure_clear_array_background(...)` — while `recording/__init__.py:50-55` claims "That indirection has been removed — every consumer and test now targets the owning submodule directly" (false). NEW seam class: `__init__.py:284-315` carries "Static-source check echo" COMMENT BLOCKS so source-inspection tests grep the echo instead of the real implementations — those tests verify nothing; and `recorder._mono_scratch_local` is a dead attribute that survives only because tests pin its presence (format.py:54-58 admits it).

**Enrichment (2026-09-04 BP session — Wave 5):** The concrete contract-pinning instance of the echo-grep pattern: tests/test_vad.py:710-726 greps `inspect.getsource(recording)` for the REMOVED 3-arg `rms_callback(chunk_rms, chunk_peak, filtered)` contract, which exists only in the __init__ echo block — the live call is 2-arg with an invariant comment forbidding the 3-arg form; test_parakeet_merge.py::TestSourceCheck greps the same echo for suppression strings. Filed as BP-136 (test masks production contract).
...
**Enrichment (2026-09-04 BP session — Wave 1; corrected Wave 2):** The fork family has a component-level triple: the LIVE Settings→Audio inline Select (`components/settings/AudioSettingsSection.tsx:371-400`), the LIVE Microphone-page accordion (`pages/microphone/components/PresetAccordionSelector.tsx:7-8`, "the labels/descriptions here mirror AudioPresetSelector's data"), and the PRODUCTION-DEAD `components/microphone/AudioPresetSelector.tsx:52-103` (zero render sites outside tests; W2-R4). All share ONE i18n key family. Filed as BP-76 (consolidation is C-MIC-4/C-MIC-15-gated: the accordion pattern is the pinned shape).
...
**Enrichment (2026-09-04 BP session — Wave 1):** React 19.2 (2025-10-01, react.dev) shipped `useEffectEvent` and `<Activity>` as stable APIs; the project pins react ^19.2.7. For the subset of the 22 `useLatestRef` mirrors that exist solely to keep effect callbacks fresh, the first-party `useEffectEvent` supersedes the bespoke mirror (upstream, linter-integrated, shrinks the migration). NOT 1:1 for `usePythonEvent`'s internal mirror or render-time `ref.current = fn` sites — per-site evaluation required. W0-verified against the react.dev 19.2 release notes.
...
**Enrichment (2026-09-04 BP session — Wave 1):** Concrete drift proof that the dead parakeet fallback copy is already diverging: `parakeet_engine/_helpers.py:91-109` `_local_is_cuda_error` lacks the ORT-1.28+ `RuntimeException`-relocation guard that the canonical `asr_utils.is_cuda_error` carries (`asr_utils.py:158-163`); plus `parakeet_engine/_constants.py:11,65-66` re-declares three asr_utils constants (0.30/2/3) as bare literals. This is the "will not be kept in sync" prediction coming true.
...
**Enrichment (2026-09-04 BP session — Wave 1):** (1) Four UNPORTED Electron surfaces that die with the shell taking live features: `src/main/power.ts` (powerMonitor suspend/resume backend recycling — no Tauri counterpart), `src/main/ipc/stats-image-handlers.ts` (stats image save/copy/reveal — absent from src-tauri and the tauri-bridge), `backend-restart-handler.ts` + `restart-backend.ts` (renderer-initiated "Lost connection" restart escalation — `WindowBridge.restartBackend` is optional and unimplemented on Tauri), and `shortcuts/global-shortcuts.ts` (the Ctrl/Cmd+Shift+D dismiss accelerator). Renderer degrades silently on Tauri for each. (2) Sequencing hazard: `autostart/tauri_spawn.py:21-24`, `autostart/focus.py:14-17`, AND `autostart/log_files.py` import `_launcher_child_env`/`_log_sensitive_env_keys`/`_spawn_flags` from `_electron_build.py` — deleting that module per ADR-0020's checklist BREAKS the Tauri autostart path (three dependents, not two); extract the helpers to a runtime-neutral module FIRST. (Corrected in Review Wave 2 — the helper names here were initially mis-cited.) (3) `app_lifecycle.py:257-301` — the entire "STANDALONE IN-PLACE RESTART" branch gates on `_electron_pid is not None`, which is always None in Tauri-WS mode: dead in the going-forward runtime, to be deleted with its plumbing. (4) Verified already-ported column: export.rs, system_cmds/{locale,renderer_log}, commands/bubble/*, window_close.rs, tauri-plugin single-instance, allowlist.rs.

**Enrichment (2026-09-04 BP session — Wave 3):** Add one decide-and-drop item: the main-process logging-health ring buffer (rotation.ts:80-148, getLoggingHealth) is Electron-main-only, unported, and unwired (zero consumers) — its disposition (wire an IPC surface vs delete at cutover) must be chosen explicitly, not left. Also: consolidate the console forwarder to the shared helper BEFORE any port decision (renderer-telemetry duplicates bubble/console-forwarder byte-for-byte — BP-105).

**Enrichment (2026-09-04 BP session — Wave 5):** Two more checklist additions: (1) the Python single-instance subsystem (single_instance.py + _security_attributes.py + security/win32_dacl.py, ~1,176 LOC) is gated OFF in Tauri mode (entrypoint.py:403) but absent from the decommission list — extract the three LIVE symbols FIRST (`_is_pid_alive`, `_backend_pid_file`, `_clear_backend_pid_file` — NOT `_write_backend_pid_file`, which is gated-off too; corrected in Review Wave 6); filed as BP-144. (2) Four more Tauri-path residue sites: focus.py:67 sets VT_FOCUS_ONLY=1 (zero src-tauri readers), autostart.py:148-150 docstring still describes spawning "npm run dev… Electron", desktop_shortcut.py docstrings cite the "Electron single-instance lock", _phases_late.py:621 logs "opening Electron window".
...
**Enrichment (2026-09-04 BP session — Wave 1):** Nine more verified sites: Rust — (1) `heartbeat.rs:128-129` claims a "120s internal timeout" that is 15s and never fires (see BP-37); (2) `tray.rs:264-278` "Python publish path NOT YET WIRED" — stale, it is wired, allowlisted and consumed; (3) `allowlist.rs:250-255` references nonexistent `sidecar/ws.rs::ws_reader_loop` and a wrong relaunch_ack route; (4) `commands/bubble/commands.rs:620-622` contradicts dispatch_fire_and_forget's doc; (5) `release_mode.rs:29-30` claims prctl PDEATHSIG while posix.rs implements a reaper script with kill -9; (6) `rate_limit.rs:38-41` "~30 clicks/sec" arithmetic wrong (max 2/sec); (7) capabilities/main-runtime.json still cites the removed clipboard-manager plugin. Python — (8) cloud `_StreamingMultipartBody` docstrings claim single-chunk peak memory while `parts.append(wav_bytes)` keeps the full WAV resident (only the second copy avoided); (9) `text_cleanup/_engine.py:46-54` claims CPython re compiles alternations to a trie — it does not (verified; trrex/Flashtext exist for that).

**Enrichment (2026-09-04 BP session — Wave 5):** Tooling-layer additions to the sweep: ruff_ratchet_check.py docstring `()` artifacts; mypy_ratchet_check.py:116 prints a literal `{proc.returncode}` (missing f-prefix); sync_versions.py docstring claims CHANGELOG syncing that doesn't exist; pyproject's "intentional divergence" matrix says `.python-version 3.12.7` vs the actual 3.13.7 pin; cloud/_providers/openai.py:23 `PERF-:` mangled tag; llm_polish.py:254-277 paste-artifact paragraph; hooks/models comments contradicting landed code (useOfflinePackDownload.ts:34-43, useCloudProviders.ts:217-230). All folded into BP-146/149/151 where structural.
...
**Enrichment (2026-09-04 BP session — Wave 1):** Six new suppression/type-debt sites, all fixable without behavior change: `service/privacy.py:316,767,802,834` — collaborators typed as bare `object` + 4× `# type: ignore[attr-defined]`, deletable via a `typing.Protocol` with write/close; `service/update_check.py:850` — arg-type suppression on the event_bus fallback (type the `ModuleType | None` resolution); `transcription.py:71` — doubled `# noqa: F401  # noqa: F401`. See BP-50 for the ASR-side micro-batch and BP-46 for the Protocol drift.
...
**Enrichment (2026-09-04 BP session — Wave 1):** New instances beyond the original six: (1) the model-load success ritual is copy-pasted across SIX load paths with i18n drift — large enough to be its own entry, BP-41; (2) token-key normalization exists 3× (memoized in text_cleanup, uncached duplicate in streaming.py:517-524, inline uncached per-token in vocabulary.py:924 — which imports neighboring helpers but not the memoized one); (3) `_hf_cache_cleanup.py:60-134` exposes FOUR names for one implementation; (4) `change_model`'s backend if/elif duplicated verbatim (folded into BP-41's fix).

**Enrichment (2026-09-04 BP session — Wave 3):** Two extensions: (1) offline_pack.check_offline_pack_disk_space re-implements asr_utils._check_disk_space_for_download while its docstring claims it wraps it (two disk-space recipes that can drift — a shared ensure_disk_space helper also gives BP-115 a canonical gate); (2) _hf_cache_cleanup is a 3-layer facade whose docstrings claim consumers that no longer reference it (parakeet_engine: zero references) and asr_utils functions it no longer contains — collapse to direct imports. See BP-120.

**Enrichment (2026-09-04 BP session — Wave 5):** New DRO instance: log/__init__.py:101-104 claims `get_logs_dir` is the single source of truth for the logs directory, but 5 production writers inline `/ "logs"` (crash_handler/_memory_buffer.py:247, ipc_diagnostics.py:155, diagnostics_export.py:150, service/privacy.py:416, native_hotkeys/_spawn.py:318).
...
### BP-39 — Rust micro-batch: ungated terminal-line format, hot-path clones, per-event spawn debounce, regex-pinned dead twins
**Status:** ❌ Not Fixed (investigation only)

**Description:** Four verified small defects: (1) `combined.rs:132` builds the terminal log line before checking whether stderr logging is even enabled — one wasted allocation per log line in every release build; (2) `reader.rs:226,266` deep-clones the payload in the two single-emit branches although it could move it — one avoidable `serde_json::Value` clone per emitted frame on the highest-rate host path (bubble_level, ≤30Hz, for the bubble window's lifetime while recording); (3) `persisted_position.rs:168-190` debounces window-move persistence by spawning a fresh async task per `WindowEvent::Moved` (~60/sec during a bubble drag → ~30 sleeping tasks); a single long-lived task parked on a `tokio::sync::Notify`/watch channel carrying the latest (x, y) would eliminate the per-event spawns (NB: this is a NEW pattern for this codebase — the nearest precedent, tray.rs:242-261, replaced per-event std::thread spawns with pooled `spawn_blocking`, which is still per-event; no Notify-debounce exists in src-tauri today); (4) `spawn_sidecar_and_get_port` and `SUPERVISOR_MAX_RETRIES` are dead production symbols kept solely so Python source-regex gate tests keep matching — coupling Rust layout to Python test regexes and blocking E15 cleanup.

**User Impact:** Individually invisible; collectively they are pure waste on hot paths and misleading structure. (1)-(3) cost CPU/allocations during recording and dragging; (4) means a rename silently breaks a Python gate test.

**Root Cause:** Mechanical shortcuts + a cross-language test seam that greps source text instead of behavior.

**Gain vs Trade-off:** Pure improvement; (4) requires repointing the mig*/gate tests at live entry points first.

**If We Do It:** Less allocation churn on the busiest host paths; dead symbols gone; gate tests test behavior, not text.

**If We Don't:** Cumulative micro-waste and a fragile test seam persist.

**My Recommendation:** ✅ Implement — batch of small, low-risk fixes.

**Progress:** `None yet.`

**Related Files:**
- `src-tauri/src/platform/logging/combined.rs:132,143-145`
- `src-tauri/src/sidecar/ws/reader.rs:226,266`
- `src-tauri/src/commands/bubble/persisted_position.rs:168-190`
- `src-tauri/src/sidecar/spawn.rs:122-137`, `src-tauri/src/util.rs:50-58`

**Fix:** (1) move the `term_line` format! inside the stderr-enabled branch; (2) `payload` move instead of `.clone()` in the two single-emit branches; (3) one long-lived debounce task parked on `tokio::sync::Notify` carrying latest (x,y), each Moved only stores + notifies; (4) repoint tests/tauri/* gate tests to `spawn_sidecar_and_get_port_with_shutdown` + the `SUPERVISOR_BACKOFF_MS` invariant already pinned in util_tests.rs, then delete the dead twins.

**Simplified Fix:** A batch of small tidy-ups: skip formatting text nobody will print, stop copying data that isn't reused, use one persistent helper task instead of spawning a new one per mouse move, and delete two leftover functions that tests only find by grepping their names.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low
### BP-40 — Model download reports success when huggingface_hub is missing (swallowed ImportError → green toast, no model)
**Status:** ❌ Not Fixed (investigation only)

**Description:** In the whisper-family download path, the `from huggingface_hub import snapshot_download` import is wrapped in a try whose `except ImportError:` only logs a debug line saying it will "fall back to engine.load()" — a fallback that was deleted in an earlier change (the comment describing it was left behind). Control then falls through to the code that reports SUCCESS: a 100% progress push, a "downloaded successfully" notification, a status-cache invalidation, and `{"success": True}`. The outer dispatcher never sees the ImportError.

**User Impact:** In an environment where huggingface_hub is broken or missing (broken install, stripped venv), clicking Download shows a green toast and full progress, the model appears downloaded — and the first dictation with it fails with an unrelated engine-load error. A silent failure dressed as success.

**Root Cause:** The ImportError is caught at branch level and the branch still returns the success payload; the fallback it used to justify that return was removed without revisiting the except arm.

**Gain vs Trade-off:** Pure improvement — mirrors the error reporting the Parakeet path already has; adds a regression test. No behavior lost.

**If We Do It:** Download reports a clear failure ("library missing") and no success toast when the dependency is unavailable.

**If We Don't:** The worst-case install keeps lying to the user.

**My Recommendation:** ✅ Implement — cheap, high-trust-value fix.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/service/model/_downloads.py:456,794-839`

**Fix:** In the except arm, return a structured failure (whisper-specific message shaped like the Parakeet path's reason table), delete the stale "falling back to engine.load()" comment, and add a regression test that monkeypatches `huggingface_hub` to be unimportable and asserts `success: False` + no 100% progress push. While there: the success path double-pushes 100% progress (:790 and :807) — fold the duplicate push into the same fix (W2-R2).

**Simplified Fix:** When the download library is missing, the app should say the download failed instead of showing a success message — and the code comment describing an old fallback that no longer exists should go.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-41 — Six copy-pasted model-load "success rituals" with i18n drift (English tray messages on 3 of 6 paths)
**Status:** ❌ Not Fixed (investigation only)

**Description:** After a model finishes loading, six separate code paths each hand-roll the same sequence (progress closure, `touch_model`, `_evict_lru_model`, clear deliberately-unloaded flag, tray "Ready" message). Three of the six use the localized `state.model_manager.ready_*` keys; the other three hardcode English strings ("Ready -- {device_info}"). One site uses the whisper key even for non-whisper engines.

**User Impact:** Non-English users see an English tray message on some model-switch paths (backend switch, fallback after failed load, reload after idle unload) and a translated one on others — inconsistent model-switch UX, depending on which internal path happened to run.

**Root Cause:** The load paths were split into mixins without extracting the shared completion ritual; each copy drifted independently.

**Gain vs Trade-off:** Mostly pure consolidation; the only behavior change is fixing the 3 English sites to use keys that already exist in all 8 locales (a bug fix, not a regression).

**If We Do It:** One helper owns the ritual; every model-ready transition shows the localized message.

**If We Don't:** Every future tray-message change must be hand-synced across 6 sites — two have already drifted.

**My Recommendation:** ✅ Implement — DRY with a user-visible consistency win.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/model_manager/_loading.py:147-183,409-440,480-506,659-698`
- `voice_typer/server/model_manager/_change.py:348-387,691-713`

**Fix:** One `_on_load_success(backend: str)` helper on the manager core owning touch+evict+clear+i18n tray message; each of the 6 load paths calls it once. Collapse the duplicated backend-name if/elif (2 verbatim copies) into `_backend_for_model_size()`. Tests: existing behavior pins + one assertion that all paths produce a localized message.

**Simplified Fix:** The "model is ready" celebration is copy-pasted in six places with slightly different wording; make them all call one shared routine so the message is consistent and translated everywhere.

**Implementation Difficulty:** 🟡 Medium
**Severity:** 🟡 Medium

### BP-42 — Prewarm's optimized file walker is built and tested — but production never calls it
**Status:** ❌ Not Fixed (investigation only) — fourth Fast-Startup defect beyond BP-7's three

**Description:** An optimized directory walker (`_iter_warmable_files`, using `os.scandir` with cached d_type to avoid one stat() per file) exists in prewarm/cache_probe.py, with a docstring saying it "replaces the old rglob('*')+is_file() pattern", and a regression test pinning its stat count. Production `_warm_package_files` still uses the OLD rglob pattern (fresh stat per entry, plus per-file Path/parts allocations). The regression test guards a helper production never executes.

**User Impact:** Every Fast-Startup (prewarm) run over the worker's warm-packages (onnxruntime, ctranslate2, numpy, scipy, faster_whisper — tens of thousands of files) pays a per-file stat + allocations the optimized walker was built to eliminate. Startup warm-up is slower than designed.

**Root Cause:** The optimization landed without being wired into the production loop; the test pinned the helper, not the wiring.

**Gain vs Trade-off:** Pure improvement — the helper already exists and is tested; wiring is a two-line change plus a wiring pin.

**If We Do It:** Prewarm file warming costs measurably less per start.

**If We Don't:** The "fixed" stat-count regression test keeps passing while guarding nothing.
...
**Related Files:**
- `voice_typer/server/prewarm/cache_probe.py:102-168` (helper), `:211-236` (production loop)
- `tests/test_cache_probe_stat_count.py`

**Fix:** Replace the inner loop of `_warm_package_files` with `for path in _iter_warmable_files(root): total += _warm_file(path)` — BUT the walker currently has NO skip-dir pruning (`tests/`, `docs/`, `__pycache__`, `*.dist-info`/`*.egg-info` — the production loop performs it at the consumer), so the skip-dir filter MUST move into the walker (or stay at the consumer); deleting it outright would warm MORE files than today (a behavior change and possible startup regression — caught in Review Wave 2). Keep the existing stat-count test, add a behavioral parity test pinning the warmed-file set, and add a call-graph/source pin asserting `_warm_package_files` references `_iter_warmable_files` so the wiring can't silently regress again. (Related: BP-7 covers the other three prewarm defects.)

**Simplified Fix:** A faster way of walking the program's files was written and tested, but the startup warm-up still uses the slow old way — switch it over and add a check that it stays switched.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-43 — Worker WS server ships a dead "DoS protection" constant that protects nothing
**Status:** ❌ Not Fixed (investigation only)

**Description:** `voice_typer/worker/_ws_server.py` defines `_MAX_WS_CONNECTIONS = 4` with a comment calling it a concurrent-connection limit for DoS protection. Repo-wide, nothing references it (the sidecar's similarly-named constant is a different module). An in-file note already concedes `serve()` has no such parameter and no semaphore is used.

**User Impact:** None functionally — but any reader or future security review will assume a connection cap exists. It doesn't. Auth, loopback-only bind, and the ephemeral port are the real (documented) controls.

**Root Cause:** Declared intent never implemented; comment outran code.

**Gain vs Trade-off:** Pure hygiene — deletion removes a false security signal; implementing a semaphore would add complexity for a single-client-by-design server.

**If We Do It:** Code stops claiming a protection it doesn't have (either by deleting the claim or actually implementing the cap).

**If We Don't:** The misleading constant keeps inviting false assurance.

**My Recommendation:** ✅ Implement (delete the constant + comment; keep auth/bind as the documented controls).

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/worker/_ws_server.py:66-70,494-502`

**Fix:** Delete `_MAX_WS_CONNECTIONS` and its "DoS protection" comment; adjust the adjacent design note. (Alternative, if a cap is genuinely wanted: an `asyncio.Semaphore(_MAX_WS_CONNECTIONS)` around `_handle_connection` — prefer deletion given the single-client respawn design.)

**Simplified Fix:** Remove a leftover setting that claims to limit how many programs can connect at once but is never actually used.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-44 — CTranslate2 engines never set cpu_threads: CPU dictation under-uses the machine
**Status:** ❌ Not Fixed (investigation only)

**Description:** Every `WhisperModel` in the codebase is constructed without `cpu_threads` (repo-wide grep: zero matches for cpu_threads/intra_threads). CTranslate2 defaults to 4 intra-op threads; Parakeet/Qwen (ONNX Runtime) scale with hardware by default. So the two engine families behave differently on the same 8-16 core machine, and the faster-whisper CPU path — the primary non-GPU path and the entire designed GPU→CPU fallback chain — leaves most cores idle.

**User Impact:** CPU-only dictations are slower than the hardware allows; identical machines give different engine speed impressions.

**Root Cause:** Constructor option never passed (verified CTranslate2 docs: intra_threads default = 4).

**Gain vs Trade-off:** Gain: CPU transcription uses available cores (magnitude not measured in sandbox — needs a bench datapoint; W2 check pending). Trade-off: more threads can contend with the audio pipeline's real-time path if uncapped — hence a sane cap (e.g. min(8, physical cores)) and inter_threads=1.

**If We Do It:** CPU dictation latency drops on multi-core machines (amount to be measured by the existing bench).

**If We Don't:** CPU users keep a 4-thread ceiling regardless of hardware.

**My Recommendation:** ✅ Implement — a library option (W2-compliant), low risk with a cap.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/transcription.py:529-533`
- `voice_typer/server/transcription_device.py:57,95-96`

**Fix:** Pass `cpu_threads` derived from physical cores (`os.sched_getaffinity`/psutil, capped ~min(8, physical)), `inter_threads=1`; add a bench datapoint (bench_transcription.py) and an E6 test pinning the constructor kwargs. VALIDATE ON WINDOWS HOST (perf).

**Simplified Fix:** Tell the speech engine how many processor cores it may use, instead of letting it default to four on every machine.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-45 — Qwen chunk-boundary dedup is a second, weaker implementation of asr_utils' canonical one
**Status:** ❌ Not Fixed (investigation only)

**Description:** Long (chunked) dictations need overlapping-chunk boundary dedup so words at chunk seams aren't duplicated. Parakeet uses the canonical `asr_utils.compute_overlap_skip` (normalized: punctuation-stripped, lowercased, window-bounded). Qwen's `_dedup_overlap` is an exact-case, punctuation-sensitive k-word equality — despite Qwen's own docstring saying it "mirrors ParakeetEngine".

**User Impact:** Chunked Qwen dictations (>30s) keep duplicated boundary words whenever the two chunk transcriptions differ in casing or punctuation; two engines silently disagree on the same problem.

**Root Cause:** Divergent re-implementation instead of importing the shared helper (E7).

**Gain vs Trade-off:** Gain: consistent dedup across engines + one implementation. Trade-off: normalization changes Qwen's output slightly (fewer duplicates) — a quality improvement, not a regression; needs a parity test with identical fixtures.

**If We Do It:** Both local engines handle chunk seams identically.

**If We Don't:** Qwen long-dictation output keeps duplicating seam words in mixed-case input.
...
**Related Files:**
- `voice_typer/server/qwen_engine.py:562-608`
- `voice_typer/server/asr_utils.py:658-765`

**Fix:** Route Qwen's boundary dedup through `asr_utils.compute_overlap_skip` (or hoist a shared normalized variant both engines import). Add a cross-engine parity test feeding identical overlapping fixtures to both engines and asserting identical seam behavior.

**Simplified Fix:** Two engines each have their own way of removing repeated words where recording chunks join; make them share the better, shared one.

**Implementation Difficulty:** 🟡 Medium
**Severity:** 🟡 Medium

### BP-46 — ASR engine protocol types misdocument the real contracts
**Status:** ❌ Not Fixed (investigation only)

**Description:** The ASR registry's `RegistryCore.transcribe_with_fallback` still annotates `audio: bytes` (`registry.py:369-371`), but every production caller passes `np.ndarray` and both whisper-family loops consume ndarray. (The `AsrBackend` Protocol at `:81-84` was ALREADY fixed to `np.ndarray` — its docstring records the bytes shape as fixed history; correction from Review Wave 2.) Separately, `transcription_load.py` requires `transcribe_words` on the engine protocol, which the CloudEngine deliberately does not implement — production duck-types via hasattr.

**User Impact:** None today (callers pass the right thing), but any new caller honoring the annotation crashes at runtime, and the Protocol misdocuments the actual hasattr-based capability contract.

**Root Cause:** Type drift after the ONNX migration; Protocol never updated.

**Gain vs Trade-off:** Pure typing hygiene — no runtime change; E9/P4 compliance.

**If We Do It:** Types match reality; new engine implementations get correct guidance.

**If We Don't:** The next engine author follows the annotation and ships a runtime crash.
...
**Related Files:**
- `voice_typer/server/asr/registry.py:369-371` (`bytes` annotation on RegistryCore.transcribe_with_fallback), `:81-84` (AsrBackend Protocol — already np.ndarray)
- `voice_typer/server/transcription.py:781-785`, `voice_typer/server/transcription_load.py:44-58`
- `voice_typer/server/dictation_pipeline/transcribe_step.py:313-316` (caller)

**Fix:** Change the annotation to `np.ndarray` (TYPE_CHECKING import already present); split `transcribe_words` into an optional-capability Protocol (or annotate the duck-typing explicitly at the consumer). Add a mypy/pyrefly-visible parity test.

**Simplified Fix:** The instruction sheet for speech engines says audio arrives as raw bytes, but it actually arrives as a numeric array — fix the sheet before the next engine author follows it into a crash.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-47 — Dead transcribe_batch API + stale orchestrator comment (Qwen)
**Status:** ❌ Not Fixed (investigation only)

**Description:** `qwen_engine.py`'s `transcribe_batch` has zero production callers (tests only) and self-describes as "forward-looking". Meanwhile a comment in `dictation_pipeline/orchestrator.py:157-160` references "ParakeetEngine._transcribe_batch" and "_AbortStoppingCriteria" — neither is functional post-ONNX-migration (a deliberate no-op shim for the latter still exists in `parakeet_engine/_shims.py:18-19`).

**User Impact:** None — dead API surface plus a comment describing machinery that isn't there.

**Root Cause:** Post-migration cleanup never reached this API/comments.

**Gain vs Trade-off:** Pure E15 hygiene.

**If We Do It:** One less dead public API; comments describe real code.

**If We Don't:** Readers keep chasing a batch API and abort classes that don't exist.
...
**Related Files:**
- `voice_typer/server/qwen_engine.py:637-670`
- `voice_typer/server/dictation_pipeline/orchestrator.py:157-160`

**Fix:** Delete `transcribe_batch` with its tests (record in archive/deleted_files.txt), fix the orchestrator comment.

**Simplified Fix:** Remove an unused batch-transcription function and correct a note that points at code that was deleted long ago.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-48 — Grammar-correction map flips "were"→"we're" and "its"→"it's" (meaning inversion)
**Status:** ❌ Not Fixed (investigation only)

**Description:** The "fix grammar basics" pass (enabled BY DEFAULT — `fix_grammar_basics: bool = True` in config/_schema.py:756; correction from Review Wave 2, it is NOT opt-in) contains whole-word IGNORECASE replacements "were"→"we're" and "its"→"it's". "Were you at the store?" becomes "We're you at the store?"; "the dog bit its tail" becomes "it's tail". The in-code acceptance note reasons only about contraction frequency, not the ubiquitous question form or the possessive.

**User Impact:** With the grammar pass on (the default), common sentences get silently reworded into wrong meanings — contradicting the module's own "MUST NOT change the meaning" contract. Every default user is exposed, not just opt-in users.

**Root Cause:** Ambiguous homophone entries accepted with a frequency-based rationale that ignored context forms.

**Gain vs Trade-off:** Gain: no meaning inversions. Trade-off: the enhancement stops auto-correcting the two most common contraction typos — a quality trade the user should decide (entries are a documented in-code trade-off).

**If We Do It:** "Were you…" questions and possessive "its" survive enhancement untouched.

**If We Don't:** Opt-in users get occasional mangled sentences.

**My Recommendation:** 🟡 Try and revert — drop the two entries (safest), revert if users miss the correction. Severity raised from Low to Medium in Review Wave 2 (default-on exposure + meaning inversion).

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/ai_enhancement.py:112-113,130-133,441`

**Fix:** Remove the two ambiguous entries (or context-gate them); add E6 regression tests with the corrupted sentences as inputs.

**Simplified Fix:** Stop the grammar helper from "fixing" words that are usually correct as-is — "were" and "its" — because the fix sometimes rewrites the sentence into the opposite meaning.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low


**Enrichment (2026-09-04 BP session — Wave 5):** Two more ambiguous entries in the same map: "ill"→"I'll" and "id"→"I'd" ("she felt ill" → "she felt I'll"; "enter your id" → "enter your I'd" — empirically reproduced). The same module's auto_capitalize also ships "may"/"march"/"august" as unconditional capitals — filed as BP-132 with all five removals coordinated.
### BP-49 — Correction-usage flush runs a full atomic file save between transcription and paste
**Status:** ❌ Not Fixed (investigation only)

**Description:** The dictation path records vocabulary-correction usage and flushes (json.dumps + atomic write + .bak of the whole usage file, under a lock) synchronously when the 5s debounce window elapses — i.e., occasionally the dictation that closes the window pays a full file save between "transcription done" and "text pasted".

**User Impact:** A periodic small latency spike right before paste; grows with vocabulary size/install age. Small at typical sizes, but it sits on the most feel-sensitive path in the app.

**Root Cause:** Debounced flush executes inline on the dictation path instead of a background sweeper.

**Gain vs Trade-off:** Gain: paste latency no longer contains file I/O. Trade-off: a daemon sweeper thread (the repo already uses this pattern elsewhere); snapshot reads keep their forced flush.

**If We Do It:** Paste timing becomes uniformly tight.

**If We Don't:** Occasional users feel a stutter before text appears.
...
**Related Files:**
- `voice_typer/server/correction_usage.py:158,170,218-237`
- `voice_typer/server/vocabulary.py:932-934` (dictation-path trigger)
- `voice_typer/server/dictation_pipeline/storage_step.py:150`

**Fix:** Move the debounce flush to a daemon sweeper thread (flush-on-debounce, plus forced flush in get_snapshot); E6 test asserting the dictation path never blocks on save. Schema untouched (C-PERSIST-2 respected).

**Simplified Fix:** Move the periodic "save usage counts" file write off the typing path so the text always appears immediately after you stop speaking.

**Implementation Difficulty:** 🟡 Medium
**Severity:** 🟢 Low

### BP-50 — ASR/text DRY micro-batch (5 items)
**Status:** ❌ Not Fixed (investigation only)

**Description:** Five verified small duplications/wastes on the text path: (a) token-key normalization exists 3× — a memoized `_token_key` (lru_cache 4096) in text_cleanup, an uncached duplicate pattern in streaming.py, and an inline uncached per-token copy in vocabulary.py (which imports neighboring helpers but not the memoized one); (b) an identical `vad_parameters=dict(...)` literal duplicated in both whisper loops; (c) `templates.py` copies the whole contains-list per dictation though the rebuild only reassigns; (d) `best_of=engine.best_of` with hardcoded `temperature=0.0` is a silent no-op (faster-whisper: best_of only applies when sampling with non-zero temperature) — the config knob does nothing; (e) a doubled `# noqa: F401  # noqa: F401`.

**User Impact:** (d) is user-visible in the sense that the "best_of" setting has no effect; the rest is drift surface and micro cost on the dictation hot path.

**Root Cause:** Mechanical duplication across sibling modules.

**Gain vs Trade-off:** Pure improvement; (d) either honors best_of (needs non-zero temperature — sampling change!) or removes the dead knob — flag: honoring it changes decoding behavior, so removal/documentation is the safe route.

**If We Do It:** One memoized normalizer, one VAD-dict constant, no per-dictation copy, honest best_of handling.

**If We Don't:** Micro-costs and a config placebo persist.

**My Recommendation:** ✅ Implement (with (d) resolved as remove-or-document, not silently-changed decoding).

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/text_cleanup/_engine.py:604-617`; `voice_typer/server/streaming.py:517-524`; `voice_typer/server/vocabulary.py:924`
- `voice_typer/server/transcription_result.py:170-173,379-382,167,376`
- `voice_typer/server/templates.py:712`
- `voice_typer/server/transcription.py:71`

**Fix:** (a) import the memoized `_token_key` at the two uncached sites; (b) hoist the VAD dict to one constant; (c) drop the list copy; (d) remove `best_of` from the kwargs (or document that it only matters with temperature>0 and gate it there); (e) collapse the doubled noqa.

**Simplified Fix:** Five small copy-paste cleanups in the text-processing code, one of which removes a settings knob that currently has no effect.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-51 — PII redaction filter runs twice per log record (~30-35% of per-line logging cost)
**Status:** ❌ Not Fixed (investigation only)

**Description:** One `PIIRedactionFilter` instance is attached to BOTH the file handler and the stderr handler. Python fires handler filters once per handler on the SAME LogRecord — the second pass re-runs the full redaction scan on already-redacted text. Measured in-sandbox: filter cost 16.3-18.6µs per record, and the fast-trigger rarely short-circuits because `\d{3,}` matches nearly every operational line (ports, durations, PIDs). End-to-end logging costs ~51.6µs/line with both handlers vs 28.6µs file-only.

**User Impact:** Invisible per line, but every INFO+ line pays ~16-19µs of duplicate CPU — roughly a third of total logging cost — scaling with log volume during dictation storms. Review Wave 2 initially reported a THIRD filter attachment (crash buffer) implying 3× scans; **Wave 3 (W3-A4) corrected this**: the crash buffer's MemoryHandler.handle() override bypasses filter application entirely, so the third attachment is attach-only and never scans — the real per-record cost is 2 passes (file + stderr), and an in-sandbox re-benchmark measured the second scan at ~41% of end-to-end per-line cost. Medium severity holds on the aggregate-win argument (a measured ~30-40% reduction across ALL logging). The crash-buffer bypass itself is filed separately as BP-93.

**Root Cause:** No idempotence guard: `filter()` re-redacts a record whose `redacted_msg` is already set by the first handler's pass.

**Gain vs Trade-off:** Pure improvement, byte-identical output (redaction is idempotent); SEC-003 unaffected (both handlers still filtered).

**If We Do It:** Per-line logging cost drops ~30%.

**If We Don't:** Cumulative waste keeps scaling with verbosity.

**My Recommendation:** ✅ Implement — two-line guard, measured win.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/security/redaction.py:932-952`
- `voice_typer/server/log/__init__.py` (~L795, ~L955 — both addFilter sites)

**Fix:** Add an idempotence guard at the top of `PIIRedactionFilter.filter`: `if getattr(record, "redacted_msg", None) is not None: return True`. Keep both addFilter attachments (SEC-003 unchanged). Add a timing/behavior regression test (double-attached filter runs the scan once).

**Simplified Fix:** The privacy filter that scrubs personal data from logs runs twice on every log line because two parts of the app each ask for it; make it notice it already scrubbed that line.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-52 — Dead config-change listener infrastructure invites contributions that never fire
**Status:** ❌ Not Fixed (investigation only)

**Description:** `event_bus.py` carries ~200 lines of `ConfigChangeListener` / `subscribe_config_changes` / `_publish_config_change` infrastructure. Repo-wide, the only production references are the definitions — no production code ever publishes or subscribes (tests only). config_applier was meanwhile refactored to the live `ConfigSideEffect` protocol + handler-list mechanism (`config_applier.py:373+`), superseding the plan the comment still describes. The docstring actively tells future contributors to "implement this protocol and register via subscribe_config_changes rather than adding a new branch" — which produces listeners that never fire.

**User Impact:** None directly — it's a contributor trap: follow the documented advice and your feature silently does nothing.

**Root Cause:** The promised follow-up wiring never landed and was superseded by the SideEffectRule design.

**Gain vs Trade-off:** Gain: -200 LOC from a BP-12-flagged file + honest guidance. Trade-off: none — the decoupling this infra offered is already provided by SideEffectRule.

**If We Do It:** event_bus.py shrinks; new config-reactive code follows the pattern that actually runs.

**If We Don't:** The trap stays armed for the next contributor.

**My Recommendation:** ✅ Implement (delete + redirect guidance to SideEffectRule).

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/event_bus.py:1062-1279`
- `voice_typer/server/config_applier.py:373-433` (the live mechanism)

**Fix:** Delete the ConfigChangeListener block and its two dedicated test files (E15; record in archive/deleted_files.txt); route the docstring's guidance to the SideEffectRule pattern. (Alternative: actually wire `_publish_config_change` into the post-apply fan-out — only if a real consumer is planned.)

**Simplified Fix:** Remove an unused "subscribe to settings changes" system whose instructions would lead a future developer to write code that never runs.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-53 — Encode pool is never drained on shutdown (worker threads outlive teardown start)
**Status:** ❌ Not Fixed (investigation only)

**Description:** `shutdown_encode_pool` exists, is exported, and its docstring says a matching call "must be added to _do_cleanup in a follow-up" — never added. The WS shutdown drain (`ws_drain.drain_ws_dispatch_pool`) drains the dispatch pool but never the encode pool. CPython ≥3.9 ThreadPoolExecutor workers are non-daemon and get joined via atexit — so a queued near-capacity encode (2.2ms per 1MiB frame measured) delays process exit and races subsystem teardown.

**User Impact:** App shutdown can linger briefly or race cleanup — cosmetic-to-minor, but it's exactly the race class the dispatch-pool drain was built to prevent, and the export+docstring advertise protection that isn't connected.

**Root Cause:** Follow-up wiring never landed; self-admitted in the docstring.

**Gain vs Trade-off:** Pure improvement — the API exists and is tested; wiring is one line in the drain.

**If We Do It:** Both WS pools drain bounded at exit.

**If We Don't:** Shutdown occasionally waits on the atexit join or races it.
...
**Related Files:**
- `voice_typer/server/sidecar_ws_internals/encode_pool.py:118-135`
- `voice_typer/server/shutdown/ws_drain.py`

**Fix:** Add `sidecar_ws.shutdown_encode_pool(ipc_server)` as an early (bounded ~2s) item in `drain_ws_dispatch_pool`'s item list. Extend the drain test to cover both pools.

**Simplified Fix:** When the app closes, it politely waits for one group of background helper threads but forgot to do the same for a second group — add the missing wait.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-54 — TCP pending-buffer cap inlined twice (drift hazard on exactly the failure paths)
**Status:** ❌ Not Fixed (investigation only)

**Description:** `_TCP_PENDING_BUFFER_CAP = 1000` is defined with a docstring calling it the hard cap on pending outbound TCP frames, but two of its three enforcement sites inline the literal `1000` (shutdown-path and re-merge-path checks) instead of the constant; only the third site references it.

**User Impact:** None today; a future tuning of the cap silently leaves the failure-path caps at 1000 — memory-bound drift precisely on the slow-client/shutdown paths the cap protects.

**Root Cause:** Mechanical inlining.

**Gain vs Trade-off:** Pure improvement; source-string-pinned tests reference the drop patterns, not the variable names — safe swap.

**If We Do It:** One knob actually controls all three caps.

**If We Don't:** The next capacity tuning half-works.
...
**Related Files:**
- `voice_typer/server/ipc/sender.py:100,587,765,784`

**Fix:** Replace both literals with `_TCP_PENDING_BUFFER_CAP`.

**Simplified Fix:** Two places hardcode a number that's supposed to come from one shared setting — point them at the setting.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-55 — Rate-limiter cost table carries 5 stale command entries
**Status:** ❌ Not Fixed (investigation only)

**Description:** `COMMAND_COSTS` has 79 entries; the live `_COMMAND_REGISTRY` has 74 commands. Five cost entries are for commands that no longer exist (kept "so older Electron builds that bridge these calls don't trip the DEFAULT_COST path") — while the project context is that Electron is being decommissioned. One entry even self-labels "stale per registry".

**User Impact:** None functionally; it's a misleading surface on a security-relevant tuning table and a Standing reminder of the Electron legacy.

**Root Cause:** Cost map outliving the commands it prices.

**Gain vs Trade-off:** Pure hygiene; removal is gated on the Electron decommission completing (BP-22) to avoid breaking older bridged builds.

**If We Do It:** The cost table prices exactly the live surface.

**If We Don't:** Future audits keep tripping over dead keys.

**My Recommendation:** 🟡 Defer until BP-22's Electron removal lands, then delete the 5 entries and update the parity test in the same change.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/ipc/rate_limiter.py:115-120`

**Fix:** Remove `delete_all_personal_data`, `export_diagnostics`, `export_gdpr_bundle`, `get_vocabulary_suggestions`, `test_llm_connection` from COMMAND_COSTS post-decommission; update `test_command_costs_does_not_list_unknown_commands` in the same change.

**Simplified Fix:** After the old desktop shell is retired, delete five cost entries for commands that no longer exist.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-56 — get_favorites is the only history list path without a deep-offset guard
**Status:** ❌ Not Fixed (investigation only)

**Description:** `get_recent` and `search` both `assert offset < 1000` (documented: SQLite OFFSET is O(offset); surface deep callers loudly). `get_favorites`' non-cursor branch runs `LIMIT ? OFFSET ?` with no guard, while the IPC layer would happily pass offsets up to 10,000,000 — a silent O(offset) skip scan where sibling paths would fail loudly.

**User Impact:** None today (favorites UI uses shallow offsets), but a future deep-offset caller regresses silently — on a 500K-row DB the scan is hundreds of milliseconds.

**Root Cause:** The guard was added to two of the three list paths.

**Gain vs Trade-off:** Pure improvement — consistency + a shared helper (DRY).

**If We Do It:** All three list paths share one bounded-offset contract.

**If We Don't:** The trap waits for the next consumer.
...
**Related Files:**
- `voice_typer/server/history_db_internals/search.py:598-660` vs `:334`, `:498`

**Fix:** Extract a shared `_assert_bounded_offset(offset)` and call it from all three list paths (favorites' non-cursor branch included).

**Simplified Fix:** Two of the three history-listing functions protect against wasteful deep paging; add the same protection to the third.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low
### BP-57 — CRITICAL: High-pass filter becomes numerically unstable at ≥44.1 kHz — corrupts dictation audio
**Status:** ❌ Not Fixed (investigation only)

**Description:** The audio chain's 80 Hz order-4 Butterworth high-pass casts its `b`/`a` coefficients to float32. At the pole radii this rounding is UNSTABLE at common native sample rates: pole radius 1.0059 at 44.1 kHz, 1.0128 at 48 kHz, 1.0121 at 96 kHz — and 88.2 kHz is marginally unstable too (max|pole| == 1.0 exactly; Review Wave 2 re-derivation) — so ONLY 16 and 22.05 kHz are stable. Production-class simulation in-sandbox: processing 512-sample chunks at 48 kHz through the real chain produces non-finite values in 46 of 60 chunks, first at ~149 ms; standalone HighPass output diverges with finite-phase absmax ~1e37 and inf by ~chunk 13-14 (W2-R3 re-measure). The recorder deliberately retunes to the device's NATIVE rate (auto preset enables the high-pass by default), nothing resamples to 16 kHz before the filter chain (verified by W2-R3 — the decisive falsification attempt failed), and there is no isfinite/nan-guard anywhere downstream. The audio bench reproduces the non-finite output (RuntimeWarning 3/3 runs, re-verified) and still prints PASS.

**User Impact:** With default settings on any microphone whose native rate is 44.1/48/96 kHz (the majority), dictation audio becomes NaN/inf ~150 ms into every recording — transcriptions come back empty/garbage; the microphone test's "filtered" WAV and filtered level bar are corrupted the same way. This is broken-core-flow on the default path.

**Root Cause:** float32 rounding of tightly-clustered poles (order-4, 80 Hz, high sample rate) moves them outside the unit circle; verified via tf2zpk pole analysis + end-to-end chain simulation (math is device-independent — LINUX sandbox qualifier applies to the simulation, not the arithmetic).

**Gain vs Trade-off:** Pure improvement — restores the filter to its designed behavior. The original float32 cast saved ~128 KB/s of allocations; correctness wins outright. sosfilt form is the numerically-standard alternative.

**If We Do It:** Recordings at any native rate produce finite, correctly filtered audio; the bench asserts finiteness.

**If We Don't:** Every default-preset recording on mainstream hardware is corrupt after the first ~150 ms.

**My Recommendation:** ✅ Implement — highest-priority finding of this session.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/audio_filters/highpass.py:62-63`
- `voice_typer/server/recording/_recorder_split.py:1147-1155` (native-rate retune)
- `voice_typer/server/recording/device_manager.py:1356-1374` (record at native rate)
- `bench/bench_audio_filter_chain.py` (reproduces, reports PASS)

**Fix:** Keep b/a float64 (revert the cast) OR convert to `scipy.signal.butter(..., output="sos")` + `sosfilt` (preferred: numerically safe for clustered poles). Add a post-design `max|pole| < 1` assert, finiteness regression tests at 44.1/48/96 kHz, and an `np.all(np.isfinite(out))` assertion per chunk in the bench (see BP-6 enrichment). The existing 48 kHz test builds the chain but never processes audio — extend it.

**Simplified Fix:** A background noise filter uses a math shortcut that breaks at the sample rates most microphones actually use, filling recordings with garbage after a fraction of a second — switch to the numerically safe formula and add tests that catch it.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🔴 Critical


**Enrichment (2026-09-04 BP session — Wave 3):** Equalizer verification (mandated by W2-R3): equalizer.py:153's RuntimeWarning "invalid value encountered in subtract" is PURELY PROPAGATED, not an independent EQ defect — empirically proven on numpy 2.1.3 (nan−nan is silent; only inf−inf warns ⇒ the warning requires ±inf in the EQ input, i.e. the upstream highpass's float32 overflow). EQ math is otherwise clean: no log-domain in process(), ANTIDENORMAL_EPSILON present, crossover poles stable at all NATIVE_MIC_RATES, preallocated buffers correct. The highpass remains the sole origin.
### BP-58 — Resampler's optimized path crashes on first use and silently falls back 5.7× slower
**Status:** ❌ Not Fixed (investigation only)

**Description:** `_get_resample_fir_taps()` returns a 3-tuple `(h_padded, n_pre_remove, n_pre_pad)`, but ALL THREE production call sites pass the whole tuple as `upfirdn`'s filter argument — which raises `ValueError: setting an array element with a sequence` on every call, caught, and silently fallen back to `resample_poly`. Measured: the intended cached-taps path is 13.6µs per 512-chunk vs 77.8µs for the fallback (5.7×), with bit-equal output when correctly trimmed. The "bit-identical to resample_poly" comment is false without the trim no site applies; the tests mock `upfirdn` so the numeric path is never exercised.

**User Impact:** Every audio chunk on the resample path (device rate → 16 kHz, ~16-94 Hz) pays 5.7× the intended CPU cost plus a raised-and-caught exception per call. No corruption today (the fallback is correct), but the designed optimization has never once run.

**Root Cause:** Helper return-shape change (tuple) never propagated to its call sites; exception-based fallback masked the bug.

**Gain vs Trade-off:** Pure improvement — 5.7× on the resample hot path + truthful comments + a real numeric test.

**If We Do It:** The resampler runs the path it was designed to run.

**If We Don't:** Every recording keeps paying the fallback cost and the exception churn.
...
**Related Files:**
- `voice_typer/server/recording/resampling.py:280,504,510`
- `voice_typer/server/recording/audio_pipeline.py:642,651`
- `voice_typer/server/audio_processor.py:523-524`

**Fix:** Unpack the tuple at each call site; slice `raw[n_pre_remove : n_pre_remove + expected_len]`; add a numeric-equivalence test against `resample_poly` (unmocked). Fix the false comment.

**Simplified Fix:** The fast audio-conversion shortcut has never worked — it errors out on every chunk and quietly uses a slower method; pass its outputs correctly and add a real numerical test.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-59 — Recorder uses a fixed 512-sample block at native rates → ~3× VAD/callback overhead at 48 kHz
**Status:** ❌ Not Fixed (investigation only)

**Description:** The recorder's audio block size is fixed at 512 samples regardless of device rate. At a 48 kHz native rate that is 10.7 ms chunks → 93.75 callbacks per second, ~93.75 worker iterations and ~93.75 VAD inferences per second — where all design comments assume 16-31 Hz, and VAD hysteresis timings (frame-count based) run ~3× faster than designed. The level monitor fixed this same defect for its own stream (scales blocksize with rate); the recorder didn't get the fix. (The recording ring buffer itself is dynamically sized to ~2.0 s of headroom at native rate — session_state.py:628-631 — so headroom is NOT reduced; correction from Review Wave 2, which struck the original 0.68 s/64-slot claim that belonged to the level monitor's pre-fix ring.)

**User Impact:** ~3× CPU/GIL/wakeup overhead on the most common device class (48 kHz native), Silero VAD computing 3× the intended inferences (reflect-padded 170→512 windows), and VAD timing behavior that differs by device rate. (Ring-buffer headroom is NOT affected — see correction in Description.)

**Root Cause:** Blocksize constant not rate-scaled (the sibling module's fix pattern wasn't propagated).

**Gain vs Trade-off:** Gain: ~3× less per-second overhead on the real-time path at native 44.1/48/96 kHz; VAD cadence returns to the designed 32 ms contract. Trade-off: none identified — PERF-006's zero-copy ring design is untouched (only chunk size changes); needs the xrun tests re-run.

**If We Do It:** 48 kHz devices cost the same per second as 16 kHz ones do today.

**If We Don't:** The most common hardware pays triple overhead for the app's whole runtime.

**My Recommendation:** ✅ Implement — mirrors an existing, tested fix in the sibling module.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/_audio_constants.py:59`
- `voice_typer/server/recording/stream_lifecycle.py:175,305`
- `voice_typer/server/level_monitor/monitoring.py:582-591` (the fixed sibling)

**Fix:** Scale recorder blocksize like the level monitor: `max(512, int(native_rate * 0.032))` — 1536 at 48 kHz → post-resample 512@16 kHz = exact Silero window. Keep PERF-006 ring math; re-run the audio xrun/recorder tests at 44.1/48/96 kHz.

**Simplified Fix:** The recorder reads sound in same-sized pieces regardless of the microphone's speed, making the busiest work run three times more often than designed on typical mics — size the pieces by the microphone's speed, like the level meter already does.

**Implementation Difficulty:** 🟡 Medium
**Severity:** 🟡 Medium


**Enrichment (2026-09-04 BP session — Wave 3):** The frame-count time-constant error is BIDIRECTIONAL, not just 48 kHz: VAD hangover/promotion counts are unnormalized per-rate — 8 kHz BT-HFP: 15-frame hangover = 960 ms (2× the documented ~480 ms, sluggish auto-stop); 48 kHz: 160 ms (over-eager). Grey-zone promote/demote oscillation scales likewise. NEW site: AudioQualityAnalyzer.LOW_VOLUME_SUSTAINED_CHUNKS=50 ("≈3.1 s @16 Hz") = 0.53 s at 94 Hz → false low-volume warnings on 48 kHz devices. Fix direction unchanged (rate-scaled blocksize normalizes all of these).
### BP-60 — VAD housekeeping batch: bundled Silero v4 (v5 ~3× faster), redundant availability check, latency under-report
**Status:** ❌ Not Fixed (investigation only)

**Description:** Three small items: (1) the repo bundles Silero VAD v4 (ONNX + legacy .jit) while upstream v5 is ~3× faster with the same 512-new-samples cadence (though v5's 16 kHz input is 576 samples = 64 rolling context + 512 new — an input-shape change, not a pure drop-in; asset swap is a product decision); (2) `_check_vad_available()` calls `is_available()` then re-checks the model path — a predicate already checked inside `is_available()`; (3) `NoiseSuppressor.latency_ms` reports 10/16 ms but excludes the streaming resampler's group delay.

**User Impact:** (1) leaves measurable CPU on the table for the whole VAD path (pending real-ORT measurement); (2)-(3) are hygiene.

**Root Cause:** Asset age + mechanical duplication + incomplete metric.

**Gain vs Trade-off:** (1) needs a product gate (v5 model swap + state migration + measurement) — real gain, real validation cost. (2)-(3) pure.

**If We Do It:** Less VAD CPU per second of audio (amount to be measured) and honest numbers.

**If We Don't:** Nothing breaks; costs stay.

**My Recommendation:** 🟡 Defer (1) to a product decision; ✅ implement (2)-(3) opportunistically.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/server/vad.py:97,153-174`
- `voice_typer/server/audio_filters/noise_suppressor.py:705-714`

**Fix:** (2) delete `_check_vad_available`, callers use `is_available`; (3) include the resampler group delay in `latency_ms`; (1) evaluate v5 swap separately with an ORT measurement (VALIDATE ON WINDOWS HOST).

**Simplified Fix:** Update-able noise/voice-detection plumbing: drop a duplicate check, report the true processing delay, and schedule an evaluation of the newer, faster voice-activity model.

**Implementation Difficulty:** 🟢 Easy (2-3) / 🟠 Hard (1)
**Severity:** 🟢 Low

### BP-61 — Pending-outbound queue replays with null sender despite a docstring claiming the fix (rate-limit bypass)
**Status:** ❌ Not Fixed (investigation only)

**Description:** send-to-python's docstring says the pending-outbound queue captures the caller's `senderId` at push time so the flush re-sends with the SAME sender identity (and that the rate-limit bypass was the real bug). The actual flush line still passes `null` unconditionally; `entry.senderId` is written but never read. The queue is capped at 16 pending messages, bounding the exposure.

**User Impact:** None visible normally — but up to 16 replayed commands after backpressure bypass the per-renderer SEC-019 rate limit on flush, and the docstring asserts an invariant the code doesn't hold.

**Root Cause:** The documented fix was never applied (or was reverted); dead fields survived.

**Gain vs Trade-off:** Gain: docstring becomes true + rate-limit accounting correct on replay. Trade-off: none — the alternative (keep null deliberately for dead-sender hygiene) is also acceptable; the defect is the doc/code mismatch.

**If We Do It:** Replayed commands count against the original sender's rate budget.

**If We Don't:** A bounded rate-limit hole + a false invariant stay.

**My Recommendation:** ✅ Implement (either direction — but pick one and make code+doc agree).

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/client/src/main/python/send-to-python.ts:71-85,96,126`

**Fix:** Either `sendToPython(entry.msg, entry.senderId)` + a test pinning the argument (preferred — makes the doc true; stale-sender map entries are bounded and reset by the existing backpressure reset), or rewrite the docstring to document the deliberate null. Both keep ALLOWED_COMMANDS formatting untouched.

**Simplified Fix:** When the app delays and retries messages that couldn't be sent, it forgets which window asked for them, so they skip the abuse-protection counter — the code comment says this was fixed; make the code match the comment.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-62 — start-python has no idempotence guard; resume can double-spawn the backend into the single-instance mutex → app quits
**Status:** ❌ Not Fixed (investigation only)

**Description:** power.ts claims the resume handler is safe because "start-python.ts's `if (state.pythonProcess) return` early-exit" makes it idempotent — that guard does not exist anywhere (grep: only the comment). startPython() unconditionally bumps the retry generation, resets kill-escalation flags (cancelling an in-flight kill), and overwrites `state.pythonProcess`. If a suspend/resume cycle (or a spurious resume event) fires while the old backend is still alive, the spawn creates a SECOND backend, which loses the `VoiceTyperSingleInstance` mutex and triggers the early-exit dialog + `app.quit()`.

**User Impact:** After a fast system wake (or spurious resume), the whole app can quit itself with a "another instance is running" dialog — caused by its own double-spawn. Frequency suspected (not reproduced in sandbox — LINUX, no suspend cycle).

**Root Cause:** Guard documented in one file, never implemented in the other; comment masks the hole.

**Gain vs Trade-off:** Gain: resume-path idempotence, no self-quit class. Trade-off: an extra alive-check on spawn (trivial).

**If We Do It:** System wakes stop being able to kill the app.

**If We Don't:** The trap stays for every laptop that wakes quickly.
...
**Related Files:**
- `voice_typer/client/src/main/power.ts:43-47,169-174`
- `voice_typer/client/src/main/python/start-python.ts` (whole function)

**Fix:** Add the real guard to startPython(): `if (state.pythonProcess && state.pythonProcess.exitCode === null && state.pythonProcess.signalCode === null) return;` (or guard the resume handler); fix both comments. Test: resume with a live process asserts no second spawn.

**Simplified Fix:** Add the "is the background engine already running?" check that a code comment promises exists, so waking the computer can't accidentally start a second engine and shut the whole app down.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-63 — relaunch-app and restart-backend duplicate the entire bridge-teardown sequence
**Status:** ❌ Not Fixed (investigation only)

**Description:** relaunch-app.ts and restart-backend.ts contain an identical ~12-step ordered teardown (destroy socket, reset backpressure flags, clear buffers/auth/ready flags, zero retry counters, bump generation, clear heartbeat, reject all pending requests) — copy-pasted, differing only in the reject message and dev-branch extras. Any new MainState lifecycle field must be added in both places; a miss creates divergent restart paths.

**User Impact:** None today; the risk is a future restart-path divergence — the exact race class the generation/retry machinery exists to prevent.

**Root Cause:** Copy-paste lifecycle logic instead of a shared reset (E7).

**Gain vs Trade-off:** Pure improvement (create-first extraction per E1).

**If We Do It:** One function owns bridge reset; both restart paths provably identical.

**If We Don't:** The two restart paths keep drifting in silence.
...
**Related Files:**
- `voice_typer/client/src/main/python/relaunch-app.ts:282-338`
- `voice_typer/client/src/main/python/restart-backend.ts:87-123`

**Fix:** Extract `resetTcpBridgeState(reason: string)` under `python/` (create-first, re-exports preserved); call from both sites; keep branch-specific extras inline. Existing restart tests should pass unmodified (same behavior).

**Simplified Fix:** The two "restart the engine" paths each contain their own copy of the same twelve-step reset checklist — merge it into one shared function.

**Implementation Difficulty:** 🟡 Medium
**Severity:** 🟡 Medium

### BP-64 — Main-process micro-batch: mkdir per log append, dead display loop, dead state field
**Status:** ❌ Not Fixed (investigation only)

**Description:** Three verified small items: (1) `appendLogLine` runs `fs.mkdirSync(dirname, {recursive:true})` on EVERY appended line — the same function already memoizes a chmod via a Set; one to three avoidable syscalls per log write on a ~50-100µs path; (2) `isForegroundFullscreen` loops over all displays but the loop body never uses the display — on multi-display macOS it issues N native `getFocusedWindow()` calls per bubble show (every dictation start); (3) `_bubblePageReady` in state.ts has zero production readers or writers (only test fixtures keep it alive); the docstring's "pending lockstep removal" halves both landed.

**User Impact:** Invisible individually; (1) stacks syscalls on every log line, (2) adds native round-trips to every dictation start on macOS multi-display, (3) is dead surface across 20+ test fixtures.

**Root Cause:** Missing memoization (pattern exists in-file), leftover refactor loop, completed migration not cleaned up.

**Gain vs Trade-off:** Pure improvement.

**If We Do It:** Leaner logging, one native call per bubble show, less dead state.

**If We Don't:** Micro-waste and dead code persist.
...
**Related Files:**
- `voice_typer/client/src/main/logging/rotation.ts:426` (vs `:429-441` the chmod cache)
- `voice_typer/client/src/main/windows/bubble/positioning.ts:286-312`
- `voice_typer/client/src/main/state.ts:121,148`

**Fix:** (1) per-dirname existence cache mirroring `_permsVerified` (invalidate on failure); (2) drop the loop — single darwin-gated `getFocusedWindow()`; (3) delete the field + fixture entries + stale comments (compiler-verified via E1 project typecheck forms).

**Simplified Fix:** Three small cleanups in the desktop shell: stop re-creating the log folder on every write, remove a loop that repeats a check once per monitor, and delete a state flag nothing uses anymore.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-65 — Microphone level monitor started after deferred visibility never stops — mic stays open with no page active
**Status:** ❌ Not Fixed (investigation only)

**Description:** `useMicrophoneLevelMonitor`'s hidden-at-mount branch registers an `onVisible` callback that starts the monitor (setting `startedHere = true`) and then EARLY-RETURNS its own cleanup — which only removes the listener. The main cleanup that sends `level_monitor_stop` when `startedHere` is true is never registered for that effect instance. The privacy test verifies the deferred START but never asserts the post-deferred unmount STOP.

**User Impact:** Launching the app in a briefly-hidden state (normal ready-to-show) with the Microphone page persisted: the page mounts hidden, becomes visible, deferred `level_monitor_start` fires, and when the user navigates away, `level_monitor_stop` is NEVER sent. The backend InputStream stays open — the OS microphone indicator stays lit with no page active — until the user revisits Microphone or closes the app. This is exactly the leak class C-BG-1's invariant exists to prevent, on a path its fix didn't cover.

**Root Cause:** Early-return structure in the effect skips registration of the stop-owning cleanup for the deferred path.

**Gain vs Trade-off:** Pure improvement — aligns with C-BG-1's own invariant; no legit behavior lost.

**If We Do It:** The mic only stays open while the Microphone page is genuinely active.

**If We Don't:** Hidden-start users get a permanently-lit mic indicator after leaving the page — a privacy-trust failure in a privacy-first app.

**My Recommendation:** ✅ Implement — high user-trust impact.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/client/src/renderer/src/pages/microphone/hooks/useMicrophoneLevelMonitor.ts:364-420,471-500`

**Fix:** Restructure so the deferred path falls through to the shared `startedHere`-guarding cleanup (swap in the stop-owning cleanup on visibility, or track a `deferredStart` flag instead of early-returning). Add a regression test: mount hidden → visibilitychange → unmount → assert `level_monitor_stop` sent.

**Simplified Fix:** When the microphone page loads while the window is hidden and becomes visible later, the live level meter starts — but the "stop when you leave the page" wiring was never connected for that case, so the microphone stays on after you navigate away; connect the stop.

**Implementation Difficulty:** 🟡 Medium
**Severity:** 🔴 High

### BP-66 — Page roots and mega-hooks leave the E3 entry-file discipline unfinished
**Status:** ❌ Not Fixed (investigation only) — extends BP-17 beyond App.tsx

**Description:** BP-17 filed App.tsx (327 code lines) as an E3 breach. The same pattern is 2-3× larger in the page roots: Home.tsx 616 code lines (6 inline python-event handlers with branching logic, 15 useState/10 useRef/9 useEffect, consent gate + force-cancel state machine), Settings.tsx 604, Onboarding.tsx 505 (its own header still claims "~180 lines"), History.tsx 459; and in the hooks: useTheme.ts 393 code lines, useMicrophoneTestSession.ts 471, useConnection.ts 360, useOnboardingWizard.ts 423.

**User Impact:** None directly — but the least-testable surfaces of the app accrete event business logic by default: a new Home event lands inline in the page root because that's where the other six live.

**Root Cause:** The App.tsx decomposition (BP-17) never propagated to page roots and mega-hooks.

**Gain vs Trade-off:** Pure refactor, behavior-preserving (mirrors the repo's own use*Toast extraction precedent); effort is the trade-off.

**If We Do It:** Event logic lives in named, testable hooks per concern; page roots return to layout.

**If We Don't:** The decomposition debt compounds with every new feature event.

**My Recommendation:** ✅ Implement — direct continuation of BP-17's own pattern.

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/client/src/renderer/src/pages/Home.tsx` (890 total / 616 code)
- `voice_typer/client/src/renderer/src/pages/Settings.tsx` (844/604), `pages/Onboarding.tsx` (614/505), `pages/History.tsx` (584/459)
- `hooks/useTheme.ts` (797/393), `pages/microphone/hooks/useMicrophoneTestSession.ts` (789/471), `hooks/useConnection.ts` (737/360), `pages/onboarding/hooks/useOnboardingWizard.ts` (563/423)

**Fix:** Extend BP-17's extraction pattern: Home's 6 event handlers → `pages/home/hooks/use*Event` hooks (mirroring use*Toast); split mega-hooks by concern (useTheme → theme apply/persist/switch concerns); refresh the stale Onboarding header. Behavior-preserving; component tests unchanged.

**Simplified Fix:** The main pages and several oversized hooks still mix layout with event handling the same way App.tsx did before its cleanup — continue that cleanup into them.

**Implementation Difficulty:** 🟠 Hard
**Severity:** 🟡 Medium


**Enrichment (2026-09-04 BP session — Wave 3):** The page-root extractions (the good side of the pattern) consolidated mass into single large domain hooks instead of concern-split hooks: useMicrophoneTestSession 789, useMicrophoneLevelMonitor 761, useOnboardingWizard 563, useVocabulary 419, useTemplates 390, ModelStep.tsx 451. Cohesive but E3-spirit decomposition candidates (step/concern sub-hooks) — treat as the same backlog.
### BP-67 — Zoom gestures write the same config twice per tick, bypassing the documented debounce
**Status:** ❌ Not Fixed (investigation only)

**Description:** Each zoom tick (Ctrl+=/−/Ctrl+wheel) calls `set_config` with the new text size DIRECTLY, and also runs the store's `setTextSize` → 300ms-debounced save → a SECOND `set_config`. Each write triggers a backend `config_changed` push, and Home's `config_changed` handler performs a full `get_config` round-trip per event. A 10-30-tick wheel burst therefore produces N direct writes + ~1 debounced write (the debounce does coalesce bursts — W2-R4 correction; the 2-per-event figure applies only to discrete keypresses >300ms apart), each amplified by a config_changed push + get_config round-trip — still defeating the documented single-write-path design.

**User Impact:** IPC storm during zoom gestures; Home re-renders its config/hotkey surfaces per event; the documented coalescing design is bypassed.

**Root Cause:** Two write paths added independently; no dedup at the direct-call site.

**Gain vs Trade-off:** Pure improvement — one write path, the debounced one (which flushes on unmount/beforeunload).

**If We Do It:** A zoom burst produces one (or few) config writes total.

**If We Don't:** Gestures keep generating 2×-amplified IPC bursts.
...
**Related Files:**
- `voice_typer/client/src/renderer/src/hooks/useGlobalKeyboardShortcuts.ts:167-192`
- `voice_typer/client/src/renderer/src/hooks/useTheme.ts:400-473,756-762`
- `voice_typer/client/src/renderer/src/pages/Home.tsx:392-411`

**Fix:** Remove the direct `set_config` from the zoom handler; rely on `setTextSize`'s debounced save (verify flush-on-unmount/beforeunload covers the last tick), or add an explicit flush flag so exactly ONE write path exists.

**Simplified Fix:** Changing text size sends the new setting to the backend twice per step — once immediately and once after a short delay — and each of those triggers a cascade; keep only the delayed, combined write.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟡 Medium

### BP-68 — Reconnect path fetches config and throws it away (stale appStore.config after outages)
**Status:** ❌ Not Fixed (investigation only)

**Description:** useConnection's reconnect comment says "we also pull config + recording state" — the `await call("get_config")` result is discarded, never passed to `setConfig`. Only the initial mount lifecycle ever refreshes `appStore.config`; background-reconnect, the `reconnected` event, and `state_changed` never do. (The connection-toast hook refreshes only theme fields.)

**User Impact:** After a backend outage/respawn cycle, config-dependent UI (route guard, hotkey chips, platform buttons) shows pre-outage snapshots — e.g. a hotkey changed while disconnected stays stale in the UI until full reload.

**Root Cause:** Comment documented an intent the code doesn't execute; reconnect paths never grew the setConfig call.

**Gain vs Trade-off:** Pure improvement — reconnect converges the UI to backend truth (matches the initial-connect path). Trade-off: one extra config apply per reconnect (cheap).

**If We Do It:** Post-outage UI matches actual backend config.

**If We Don't:** Silent staleness after every outage.
...
**Related Files:**
- `voice_typer/client/src/renderer/src/hooks/useConnection.ts:440-491,600-609,634-660`

**Fix:** Call `setConfig(cfg)` in `tryReconnect` (make the comment true, align with initial-connect), or correct the comment and document the accepted staleness. Prefer the former.

**Simplified Fix:** After the connection to the engine recovers, the window fetches the current settings but ignores the answer — apply it, so the UI matches reality.

**Implementation Difficulty:** 🟢 Easy
**Severity:** 🟢 Low

### BP-69 — Settings search: three rebuild derivations with two match semantics
**Status:** ❌ Not Fixed (investigation only)

**Description:** Settings.tsx builds three per-keystroke derivations, each rebuilding the label universe from scratch (getSectionLabels + getPrewarmAndUpdatesLabels). The auto-switch derivation counts superstring matches (`q.includes(label)`); the empty-banner and other-groups derivations do not — so one query can simultaneously show "no settings match" AND auto-navigate to the section it matched. The `settingsAdvanced` augmentation block is also duplicated (two sites).

**User Impact:** On longer queries the page can navigate to a matching section while claiming nothing matched — contradictory UX; plus triple label-rebuild cost per keystroke.

**Root Cause:** Three derivations written independently (BP-23 filed the deep-link twins; this is the third derivation + the semantics split).

**Gain vs Trade-off:** Pure improvement; one match semantic chosen deliberately.

**If We Do It:** Banner and auto-switch always agree; one memoized label universe.

**If We Don't:** The contradiction stays reachable.

**My Recommendation:** ✅ Implement (fold into BP-23's fix scope).

**Progress:** `None yet.`

**Related Files:**
- `voice_typer/client/src/renderer/src/pages/Settings.tsx:370-407,510-519,529-549,683-687`

**Fix:** One memoized augmented label universe + a shared `matches(query, label)` predicate used by all three derivations; pick one match semantic (likely subsequence/includes — decide deliberately); collapse the duplicated augmentation. Related: BP-23.

**Simplified Fix:** Three parts of the settings page each decide "does this match the search?" in their own way and rebuild the same data per keypress — unify them.
...
## BP Session (2026-09-04) — Wave 3 additions (entries BP-79+)

> Wave 3 deepened the areas Waves 1-2 covered thinly (Rust skimmed modules, service skims, dictation pipeline, infra remainder, audio pass 2, client-main 2, bubble + page subdirs, components 2, offline-pack/update, privacy/service). All entries investigation-only; nothing implemented this session.
...
## BP Session (2026-09-04) — Wave 5 additions (entries BP-126+)

> Wave 5 = final sweep: un-owned packages (autostart/startup_sequence, credential_store, llm_polish, cloud providers, server_platform), cross-cutting themes (test-masking, IPC event parity, dead-code census, build/dev-loop wiring, renderer + Rust remainders). All entries investigation-only.
