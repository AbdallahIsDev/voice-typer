...
The 61-command table (§2) and 24-event table are the **frozen contract** for the v1 migration. If the migration introduces new commands (e.g., `tray_click` for §6.5 — already added; or `notification_clicked` for toast interactions), they MUST be:
...
- `handlers/*` — all 78 command handlers. Unchanged.
