...
#### §16 addendum 2026-08: microphone test file-reference transport

- **Added:** `microphone_test_read_audio`. The completed mic-test WAVs (~0.9 MB each at device-native rates) exceed the 1 MiB single-frame IPC cap when base64-encoded, so the old monolithic `audio_base64`+`raw_audio_base64` stop payload was silently dropped by the transport frame guard and a perfectly recorded 10 s test produced no result. `stop_test_recording` now persists each WAV under `<config>/mic-test-recordings/` and returns small `{"path","bytes"}` refs; the renderer fetches bytes via this chunked read command (each slice ≤256 KiB binary, well under the frame cap). The handler enforces recordings-dir containment (path traversal rejected), has a `_validate_dict_payload` schema, dispatch-error coverage in `tests/test_ipc_dispatch_errors.py`, and transport tests in `tests/test_mic_page_selection_and_test_transport.py`. `EXPECTED_COMMANDS` grew 69 → 70; `_COMMAND_REGISTRY`, TS `ALLOWED_COMMANDS`, and the Rust allowlist literal all grew by one in the same commit (70 shared entries).
...
- `model_manager.py`, `model_registry.py`, `asr_registry.py`, `asr_setup.py`, `parakeet_engine.py`, `qwen_engine.py`, `cloud_engines.py` ASR engine management. Unchanged by the predecessor→Tauri migration. **Subsequent ONNX migration (completed 2026-08-15, Phase 1d):** `parakeet_engine.py` was rewritten (`PLAN_ONNX_INTEGRATION.md` §3) from `transformers + torch` to `onnx_asr` (ORT backend), and Qwen is ONNX-only via `qwen_onnx_model.py` (pre-exported `andrewleech/qwen3-asr-*-onnx`, see `PLAN_ONNX_INTEGRATION.md` §4). `cloud_engines.py` and `llm_polish.py` are unaffected (zero torch/transformers/onnxruntime imports: verified).
