...
2. **Fixes the source-file line references.** The previous version cited `prewarm.py:17` for the prewarm helper; `prewarm.py` is 2,162 lines long and line 17 is part of the module docstring. References now point to **symbols** (function/class names) with a line number as a convenience only.
...
1. **Electron main process** (hosts the React UI). Source: `voice_typer/client/src/main/index.ts` (2,204 lines). Entry: `package.json` `main: ./out/main/index.js`. Build: `electron-vite build` → `electron-builder` (NSIS on Windows, DMG on macOS x64+arm64, AppImage+deb+rpm on Linux).
2. **Python backend** — `python -m voice_typer.server.ipc_server --port 9876` (1,952 lines). Spawned by `electron_launcher.py:launch_electron_frontend` (the inverse path — Python-as-parent — also exists) and reached over a local TCP socket on `127.0.0.1:9876`. Audio capture + ASR inference + tray + hotkeys + volume ducking + clipboard all live here. The IPC dispatch layer is `_COMMAND_REGISTRY` (`ipc_server.py:1347`, 68 commands) → `_handle_<cmd>` mixins in `voice_typer/server/handlers/*`. Server-initiated events flow through `event_bus.publish(...)` (`event_bus.py`, the modern successor to `ipc_server._push_event_now`).
3. **prewarm helper** — `prewarm.py` (2,162 lines). A standalone boot-time process that warms the OS file cache (~7 GB of torch + transformers + model weights) before the app's cold imports contend for disk. Kept intentionally separate per ADR-0011. Scheduling is platform-specific:
...
- **(A) IPC middleware dislike.** The UI ↔ Python path is Electron `ipcMain`/`ipcRenderer` (in `client/src/main/index.ts` and `client/src/preload/index.ts`) → TCP socket → Python `IPCServer._dispatch` → handler mixins. We want to remove the hand-rolled Electron `child_process.spawn` + `electron_launcher.py` relay layer (215 lines) and the `autostart_launcher.py` Electron-aware spawn logic (464 lines).
...
- **Crash isolation (FT-1)** is a hard requirement before cutover on each platform: Rust respawns the sidecar only, shows "reconnecting…", with backoff 500 ms → 1 s → 2 s (cap 5 retries) then full-app relaunch. The previous ADR referenced FT-1 in `.workspace/TASKS.md` — that path does **not exist** in the current repo (`rg` returns no hits under `.workspace/`). Treat FT-1 as a label defined by this ADR, not an external task ID.
...
| Audio recording | PortAudio (WASAPI / CoreAudio / ALSA / PipeWire) | Unchanged | `recording.py` stays in Python. |
...
| Volume ducking — Linux | (no native backend today — `volume_backends.py` returns a no-op) | Unchanged | Documented limitation; out of scope. |
...
- Port is **ephemeral `127.0.0.1:0`**, chosen by the sidecar and reported to Rust over stdout (see §1); auth is the existing **HMAC token** via `VOICE_TYPER_IPC_TOKEN`. Reuse `ipc_server._validate_dict_payload` (`ipc_server.py:62`) + error codes (`invalid_payload`, `missing_field`, `invalid_field`).
...
- Enable the `single-instance` plugin so only one app instance runs. **On Windows, also remove the `VoiceTyperSingleInstance` Win32 mutex from `app.py:2086`** when running under Tauri — the Tauri plugin already provides the mutex, and double-locking would block the second-instance focus path.
...
The sidecar pushes UI events through `event_bus.publish(event)` (`event_bus.py`, the modern successor to `ipc_server._push_event_now` at `ipc_server.py:389`); the Rust bridge subscribes and re-emits each as a Tauri event. This is **channel (2)** — server-initiated events, distinct from the command/response envelope (channel 1). Every event below is delivered as `{"type":<name>,"data":{...}}`. Payloads are carried unchanged from today's code so the React UI needs no reshaping.

**Verified against the live `voice_typer/server/` tree (2026-07-16).** The table below lists **21 events** — 3 more than the earlier draft (`history_changed`, `recording_started`, `recording_stopped` were previously missed). Line numbers are a source snapshot; if a referenced file changes, re-locate via the **symbol**, which is the canonical anchor — `event_bus.publish("<name>")` for events, `def _handle_<cmd>` in `handlers/*` for commands — the line number is convenience only. `ready` is emitted via `IPCServer.push` (not `event_bus.publish`) at `ipc_server.py:1899` but flows through the same channel. Empty `data` is written as `{}` (never `null`) so the Rust subscriber can always do `event.data ?? {}`.

| Event `type` | Source (file:symbol) | `data` payload | Notes |
|---|---|---|---|
| `ready` | `ipc_server.py:1899` (`IPCServer.push`) | `{}` | emitted on server start (Electron defers window creation; Tauri should likewise defer UI hydration) |
| `bubble_show` | `app.py:594` (`event_bus.publish`) | `{}` | show waveform bubble |
| `bubble_hide` | `app.py:598` (`event_bus.publish`) | `{}` | hide waveform bubble |
| `bubble_level` | `app.py:658` (`event_bus.publish`) | `{rms:float, peak:float}` | ~60 Hz source → Rust coalesce ≤30 Hz (see §9) |
| `bubble_set_state` | `app.py:693` (`event_bus.publish`) | `{state:str}` | |
| `transcription_final` | `dictation_pipeline.py:853` (`event_bus.publish`) | `{text:str (≤200 chr)}` | UI preview / refresh |
| `vocabulary_suggestion` | `dictation_pipeline.py:755` (`event_bus.publish`) | `{suggestions:[{original,corrected,confidence,context,timestamp}]}` | |
| `hotkey_capture_cancel` | `hotkey_dispatcher.py:243` (`event_bus.publish`) | `{}` | |
| `config_changed` | `config_handlers.py:144`, `service.py:1369` (`event_bus.publish`) | `{validated config updates}` | |
| `history_changed` | `history_handlers.py:197` (`event_bus.publish`) | `{reason:str}` | **added** — missed in earlier draft |
| `microphone_test_complete` | `level_monitor.py:866` (`event_bus.publish`) | `{duration:float}` | |
| `microphones_changed` | `startup_tasks.py:196` (`event_bus.publish`) | `{count:int}` | |
| `audio_clip` | `recording.py:2149` (`event_bus.publish`) | `{peak:float, count:int}` | |
| `recording_started` | `recording_controller.py:335` (`event_bus.publish`) | `{}` | **added** — missed in earlier draft |
| `recording_stopped` | `recording_controller.py:362` (`event_bus.publish`) | `{}` | **added** — missed in earlier draft |
| `download_progress` | `service.py:1654` (`event_bus.publish`) | `{model, progress(0-100), status, +optional downloaded_bytes, total_bytes, speed_bytes_per_sec, eta_seconds, paused, resumed}` | |
| `electron_notification` | `system_handlers.py:282`, `startup_sequence.py:107` (`event_bus.publish`) | `{title, message, duration_ms, critical}` | → **native toast** under Tauri (`tauri-plugin-notification`); rename to `notification` on the Tauri side |
| `navigate` | `tray.py:628` (`event_bus.publish`) | `{path:str}` | tray → UI route |
| `show_window` | `tray_window.py:123` (`event_bus.publish`) | `{}` | |
| `quit_app` | `app.py:1251` (`event_bus.publish`) | `{}` | sidecar requests app quit |
| `relaunch_electron` | `app.py:1329` (`event_bus.publish`) | `{}` | → **Tauri app relaunch** under Tauri; rename to `relaunch_app` on the Tauri side |
...
**68 commands registered (verified against `ipc_server.py:1347` on 2026-07-16)**; each maps to a `_handle_<cmd>` mixin in `handlers/*`. Dispatch is generic: `getattr(self, _COMMAND_REGISTRY[cmd])` → `(data, resp)`. Request `{"type":<cmd>,"data":{...}}`; response `{"type":"result"|"error","data":{...},"code"?}`. Exact `data` fields per command are defined inside each `_handle_*` and re-validated by `ipc_server._validate_dict_payload` (`ipc_server.py:62`) — **that function is the source of truth for command-payload shape and must be ported 1:1, not redesigned or relaxed**; line numbers drift, so locate each handler by `def _handle_<cmd>` in `handlers/*`.
...
- `prewarm.py` is frozen the **same Nuitka way** into `prewarm-<target-triple>[.exe]` (kept separate per Rule 1 — not via the sidecar's Python). One prewarm binary per target triple.
...
- **Long text (≥ ~300 chars) / text-field target:** copy via `tauri-plugin-clipboard-manager` then send `Ctrl+V` (Windows/Linux) or `Cmd+V` (macOS) via `enigo`, then optionally restore the previous clipboard. Matches today's "paste on stop" logic in `clipboard.py`.
...
**Decision: keep the native hotkey binaries, spawned by the Python sidecar (not by Tauri).** The existing `hotkeys.py::create_hotkey_backend()` factory already handles binary discovery + spawn + fallback. The sidecar (post-migration) does exactly what the Python backend does today: spawn the native binary as a subprocess, parse its line-delimited stdout wire protocol (READY / FN_DOWN / KEY_DOWN / MOD_DOWN / etc.), and match against the registered hotkey. **Tauri does not touch the hotkey subsystem at all.** This preserves ADR-0007 + ADR-0008 unchanged.
...
**Under Tauri:** the tray icon moves to `tauri-plugin-tray` (Win32 / AppKit / GTK via `gtk-3.0`), but the **menu structure and locale logic stay in the Python sidecar** — the sidecar computes the menu items and emits them as a `tray_menu` event; the Rust host renders them via the Tauri tray API. This preserves the existing `tray.py` / `tray_menu.py` logic unchanged. Click events flow back from Rust → sidecar via `invoke('dispatch', {cmd: 'tray_click', data: {item_id}})` (a new command, not in the 68-command table — see §16 for the new-command process).
...
The 68-command table (§2) and 21-event table are the **frozen contract** for the v1 migration. If the migration introduces new commands (e.g., `tray_click` for §6.5, or `notification_clicked` for toast interactions), they MUST be:
...
- **No hand-rolled launcher.** Tauri owns the Python lifecycle; the `electron_launcher.py` (215 lines) + `autostart_launcher.py` (464 lines) relay behind complaint (A) is removed. (Note: the Rust↔sidecar bridge is still a thin IPC layer — complaint (A) is addressed by removing Electron's `ipcMain`/`contextBridge` middleware, replaced by a single Tauri `invoke`→WebSocket path.)
...
- **Startup latency:** 2–5 s cold sidecar start. Mitigation: prewarm file-cache warming + background load. The existing `prewarm.py` + `prewarm_scheduler_posix.py` + `task_scheduler.py` already handle this — they keep working post-migration.
...
- `handlers/*` — all 68 command handlers. Unchanged.
- `app.py`, `service.py` — the `VoiceTyperApp` and `VoiceTyperService` domain layer. Unchanged.
- `recording.py`, `recording_controller.py`, `streaming.py`, `dictation_pipeline.py`, `transcription.py` — audio capture + ASR pipeline. Unchanged.
...
- `hotkeys.py`, `hotkey_dispatcher.py`, `hotkey_spec.py`, `native_hotkeys.py`, `native/*` — global hotkey subsystem (ADR-0007, ADR-0008). **Stays in Python; spawns the native binaries as today.** Tauri does NOT touch this.
...
- `volume_ducker.py`, `volume_backends.py`, `volume_backend_base.py` — volume ducking. Unchanged.
...
- `prewarm.py`, `task_scheduler.py`, `prewarm_scheduler_posix.py` — prewarm + scheduling (ADR-0011). **Stays in Python; `resolve_prewarm_exe()` is the only addition.**
...
- `startup_sequence.py`, `startup_tasks.py`, `thread_registry.py`, `log.py`, `branding.py`, `container_detect.py`, `_lazy_import.py`, `providers.py`, `server_platform.py`, `platform_utils.py` — infrastructure. Unchanged.
...
- `electron_launcher.py` (215 lines) — the Electron spawn relay.
- `autostart_launcher.py` (464 lines) — the Electron-aware autostart launcher.
- `_electron_build.py` — Electron build helpers.
- `client/src/main/index.ts` (2,204 lines) — the Electron main process. Replaced by Rust `main.rs` + Tauri plugins.
...
- `scripts/build/installer.iss` — Inno Setup script (Windows). Replaced by Tauri's NSIS bundler.
...
- The `VoiceTyperSingleInstance` Win32 mutex in `app.py:2086` — disabled via `TAURI_SIDECAR=1` (Tauri's `single-instance` plugin replaces it).
...
- `scripts/build/installer.iss` — Inno Setup script (REMOVED on Tauri path).
...
3. **`prewarm.py:17`** — wrong line. Line 17 is part of the module docstring. The prewarm entrypoint is the `run` function. Fixed to use the symbol name.
...
16. **`.workspace/TASKS.md`** — referenced for FT-1, but this path does not exist in the current repo. FT-1 is now defined by this ADR itself. Fixed in the Locked Implementation Decisions.
