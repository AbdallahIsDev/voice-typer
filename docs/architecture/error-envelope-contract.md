...
Voice Typer uses **one unified error-envelope contract on both IPC transports** (TCP loopback for the predecessor host, localhost WebSocket for the Tauri host). The unification was finalized in CR-20 (2026-07-18), earlier drafts documented a per-transport split, but that split no longer exists.
