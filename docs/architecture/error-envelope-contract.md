...
Lausu uses **one unified error-envelope contract on both IPC transports** (TCP loopback for the predecessor host, localhost WebSocket for the Tauri host). The unification was finalized in CR-20 (2026-07-18), earlier drafts documented a per-transport split, but that split no longer exists.
...
| `server.no_model` | No speech model is selected/installed (ADR-0023 media-to-text start gate; the renderer deep-links to Models). |
| `server.job_busy` | Another media transcription job is already running (ADR-0023 single-active-job policy). |
| `server.not_supported` | The media source cannot be transcribed (no JS runtime for a YouTube challenge, live stream). |
...
- **Renderer (`usePython.ts`)**: inspects the resolved value of `window.python.call(...)` and throws a real `Error` when it sees either of the two error-envelope shapes the predecessor main process can resolve with (the synthetic `{_error: "..."}` shape for backend-not-connected / `sendToPython` failures, AND the passthrough `{type: "error", data: {code, message}}` shape for server-side dispatch errors). The throw reads `result._error || result.data?.message || "unknown error"` so `try { await python.call(...) } catch (e) {}` callers see real failures on both shapes.
- **Rust host (`dispatch` Tauri command)**: rejects the `invoke` promise on `type: "error"`, passing the sidecar's error envelope through **VERBATIM**: the command returns `Err(LausuError::server_from_data(data))` (see `src-tauri/src/error.rs`), whose custom `Serialize` emits the string `{"type":"error","data":<data>}` The envelope re-wrapped with the sidecar's `data` payload untouched, so structured fields the renderer branches on (`data.errors[]`, `consent_field`, `engine_name`, `model_id`, …) reach the webview. The rejection payload is a STRING (never an object) because the renderer-side `usePython.ts` normalizes only `typeof err === "string"` rejections. The host-side rejection is also how the renderer-side `await api.call(...)` throws before the resolved value is ever inspected. The renderer's in-code envelope checks are unreachable dead code on the Tauri path (errors surface via promise rejection); they remain in the source because the same `usePython.ts` bundle runs under both hosts and is live on the predecessor path. Log-facing strings are unchanged for host-side consumers: `LausuError`'s `Display` still renders `"server error [<code>]: <message>"` for the tray/heartbeat `log::warn!` lines, and the host-originated envelope rejections (`pending_full`, `disallowed_command`, `data_too_large`, `disallowed_window`) serialize to the same envelope JSON the former inline `Err(json!(...).to_string())` sites emitted. The renderer's `parseTauriErrorEnvelope` (`lib/python-bridge/error-envelope.ts`) parses the rejection string and stamps `code`, `errors[]`, `consent_field`, `engine_name`, `model_id` (and the Tauri-only `legacy_code` superset) with the same non-empty-string/array guards the predecessor path applies.
