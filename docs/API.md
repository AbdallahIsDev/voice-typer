...
> **Note on `unload_all()` (removed).** Earlier drafts of this document
> listed `ModelManager.unload_all()`. That method **does not exist** on
> the current `ModelManager` class (verified via `rg -n "def unload_all"
> voice_typer/server/model_manager.py` — no matches). Per-engine lifecycle
> is handled by `TranscriptionEngine.unload()` (see `voice_typer/server/transcription.py:1206`);
> the `ModelManager` exposes `ensure_active_engine_loaded()` +
> `start_background_load()` for loading and swaps engines via the
> `active_engine` setter, which transparently unloads the previous engine
> before loading the new one. To explicitly free GPU memory for a single
> engine, call `model_manager.active_engine.unload()`. There is no
> bulk-unload-all API by design — the manager only ever holds one
> engine resident at a time.
