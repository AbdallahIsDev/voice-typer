...
| `tests/test_host_ipc_and_build.py` | 90 | 0 | 0 | 90 | No rewrites done: most tests assert on Python source, `package.json`, the legacy builder config, `.github/workflows/build.yml`, or `lausu.spec` (build infrastructure, not renderer behavior). The TS-string tests in this file (e.g. `TestLegacyExposesDataExportHandlers`, `TestAllowlistCorrectness`, `TestRestartRequestRemoved`, `TestTypeScriptNonNullAssertions`) target `main/index.ts` / `preload/index.ts` which require a previous-host runtime to test behaviorally, out of scope for vitest (which runs in jsdom). Tech debt: deferred to a future previous-host test pass. |
...
- **predecessor-only paths**: tests that assert on `src/main/index.ts` /
  `src/preload/index.ts` (e.g. `TestLegacyExposesDataExportHandlers`,
  `TestAllowlistCorrectness`) are NOT candidates for vitest rewriting
   because vitest runs in jsdom (no predecessor `ipcMain`, no preload
   bridge). These need either an predecessor-focused integration test
...
cd /home/z/my-project/lausu && python -m pytest \
  ...
  tests/test_host_ipc_and_build.py -q --no-header
```
