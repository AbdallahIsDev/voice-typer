...
- Dispatch incoming WS frames through `IPCServer._dispatch` (reusing the 73-command registry unchanged)
...
The `sidecar_ws` module **reuses** the existing 73-command IPC registry from `IPCServer._COMMAND_REGISTRY`. It does NOT add any new commands. The WS transport replaces the TCP transport end-to-end:
