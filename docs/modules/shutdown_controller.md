...
The `ShutdownController` manages the entire cleanup and shutdown lifecycle of the Lausu application. It was extracted from `LausuApp` during the RW-9 god-class decomposition.
...
None. The `ShutdownController` is not directly exposed over IPC. It is invoked internally by `LausuApp.quit_app()` and `LausuApp.restart_app()`, which are themselves exposed as IPC commands (`quit_app`, `restart_app`).
