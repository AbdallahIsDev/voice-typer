...
| `voice_typer/server/ipc_server.py` (modified) | `--ws` CLI flag + `TAURI_SIDECAR=1` env gate | `--ws` sets `TAURI_SIDECAR=1` and delegates to `sidecar_ws.run()`. Under `TAURI_SIDECAR=1`: (a) `_heartbeat_loop` thread is NOT started (supervisor replaces ADR-0018); (b) `LausuSingleInstance` Win32 mutex is NOT acquired (Tauri's `single-instance` plugin replaces it). predecessor path unchanged. |
