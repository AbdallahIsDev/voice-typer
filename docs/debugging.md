...
1. Check `<DATA_DIR>/logs/voice-typer-rust.log` (and
   `<DATA_DIR>/logs/voice-typer.log` if present) for
...
- **Logs**: always attach `<DATA_DIR>/voice-typer.log` (Python) and
  `<DATA_DIR>/logs/voice-typer-rust.log` (Tauri host) to bug reports. `<DATA_DIR>` is
